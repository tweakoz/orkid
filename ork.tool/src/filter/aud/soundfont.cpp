...
void SoundFontConversionEngine::ProcessInstruments(void) {
  ...
  for (int i = 0; i < numsamples; i++) {
    ...
    // orkmessageh( "Sample %d %08x %s [len %d] [res_len %d] [start %d] [end %d] [loopstart %d] [loopend %d] [res_loopstart %d]
    // [res_loopend %d] [loopena %d]\n", i, smp, smp->name.c_str(), smp->len, smp->resample_len, smp->start, smp->end,
    // smp->loopstart, smp->loopend, smp->resample_loopstart, smp->resample_loopend, smp->loop );
  }
...
void SoundFontConversionEngine::ProcessPresets(void) {
  ...
  for (int j = 1; j <= inumpresets; j++) {
    ...
    orkmessageh("//////////////////////////\n");
    orkmessageh(
        "// program: %03d <%s> pbag_base %d num_pbags %d pr1 %d pr2 %d mapped %d\n",
        i,
  ...
  if (pengine) {
    ...
    for (int i = 0; i < inuminstruments; i++) {
      ...
      std::string InstrumentName =
          CreateFormattedString("/%s/[%d]%s", mSoundFontName.c_str(), pinst->GetIndex(), pinst->GetName().c_str());
