...
struct sfontpreset {
  ...
  std::string GetName(void) const {
    return (std::string)&achPresetName[0];
  }
};
...
struct sfontinst {
  ...
  std::string GetName(void) const {
    return (std::string)&achInstName[0];
  }
};
...
public: //
  ...
  const S16* GetSampleData(void) const {
    return mpsampledata;
  }
...
public: //
  ...
  bool IsGlobalZone(void) const {
    return (mbGlobalZone);
  }
  void SetGlobalZone(void) {
    mbGlobalZone = true;
  }

  int GetBaseModulator(void) const {
    return miBaseModulator;
  }
  void SetBaseModulator(int idx) {
    miBaseModulator = idx;
  }
...
public: //
  ...
  const SF2InstrumentZone* GetIZoneFromVelKey(int ikey, int ivel) const {
    ...
    for (size_t iz = 0; iz < inumiz; iz++) {
      ...
      if ((ikey >= ptestz.GetKeyMin()) && (ikey <= ptestz.GetKeyMax()) && (ivel >= ptestz.GetVelMin()) &&
...
public: //
  ...
  const SF2ProgramZone& GetZone(size_t idx) const {
    return mPZones[idx];
  }
  void AddZone(const SF2ProgramZone& zone) {
    mPZones.push_back(zone);
  }
...
public:
  ...
  void SetSoundFontName(const std::string Name) {
    mSoundFontName = Name;
  }

  virtual void AddProgram(sfontpreset* preset);
  virtual void AddSample(sfontsample* sample);
  virtual void AddPresetGen(SoundFontGenerator* pgn);
  virtual void AddInstrumentZone(sfontinstbag* ibg);
  virtual void AddInstrument(sfontinst* inst);
  virtual void AddInstrumentGen(SoundFontGenerator* igen);
  virtual void AddProgramZone(sfontprebag* pbg);
  ...
  size_t GetSampleBlockSize(void) const {
  ...
  size_t GetNumInstruments(void) const {
    return mPXMInstruments.size();
  }
  size_t GetNumIZones(void) const {
    return mPXMInstrumentZones.size();
  }

  size_t GetNumPrograms(void) const {
    return mPXMPrograms.size();
  }
  size_t GetNumPZones(void) const {
    return mPXMProgramZones.size();
  }

  size_t GetNumSamples(void) const {
    return mPXMSamples.size();
  }
...
protected:
  ...
  orkvector<SoundFontGenerator*> mPXMPresetGen;
  orkvector<SF2InstrumentZone*> mPXMInstrumentZones;
  orkvector<SoundFontGenerator*> mPXMInstrumentGen;
