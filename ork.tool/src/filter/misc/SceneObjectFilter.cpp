...
SceneObjectFilter::SceneObjectFilter(const CClass* pClass) : CObject(pClass) {}
...
bool SceneObjectFilterInterface::ConvertAsset(const std::string& FromFileName, const std::string& ToFileName) {
  ...
  // pscene->SetApplication(mApplication);
  {
    ...
    applicationContext->ObjectManager().DeSerializeFile(FromFileName.c_str(), deserialized, applicationContext, 0);
