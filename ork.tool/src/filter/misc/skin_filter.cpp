...
void ParseModelNode( const TiXmlElement *pmdlelem, EntitySkin & skin )
{
	...
	for(const TiXmlElement *pXmlElement = pmdlelem->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
	{
		...
		{
			throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseModelNode(): <Invalid Node Name %s>", pXmlElement->Value() );
		}
...
void ParseAnimEventSeqNode( const TiXmlElement *pelem, EntitySkin & skin, AnimSeqEventTable* ptab )
{
	...
	struct X
	{
		...
		{
			if(parsed)
				throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseAnimEventSeqNode(): Event specifies more than one action");
...
void ParseAnimNode( const TiXmlElement *pelem, EntitySkin & skin )
{
	...
	for(const TiXmlElement *pXmlElement = pelem->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
	{
		...
		{
			throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseAnimNode(): <Invalid Node Name %s>", pXmlElement->Value() );
		}
	}
...
void ParseAnimMapNode( const TiXmlElement *pelem, EntitySkin & skin )
{
	for(const TiXmlElement *pXmlElement = pelem->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
	{
		...
		{
			throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseAnimMapNode(): <Invalid Node Name %s>", pXmlElement->Value() );
		}
	}
}
...
void ParseSoundMapNode( const TiXmlElement *pelem, EntitySkin & skin )
{
	for(const TiXmlElement *pXmlElement = pelem->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
	{
		...
		{
			throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseSoundMapNode(): <Invalid Node Name %s>", pXmlElement->Value() );
		}
	}
...
void ParseSphereNode( const TiXmlElement *pelem, EntitySkin & skin )
{
	...
	skin.AddCollisionSphere( GroupName.c_str(), Joint.c_str(), Radius, Offset, TakeDamage, GiveDamage, StopMotion, Multiplier, DefaultActive );
...
void ParseCylinderNode( const TiXmlElement *pelem, EntitySkin & skin )
{
	...
	{
		skin.AddCollisionSphere( GroupName.c_str(), Joint.c_str(), Radius, Start + delta*(CReal(i)/countspheres), TakeDamage, GiveDamage, StopMotion, Multiplier, DefaultActive );
	}
}
...
void ParseCollisionMapNode( const TiXmlElement *pelem, EntitySkin & skin )
{
	for(const TiXmlElement *pXmlElement = pelem->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
	{
		...
		{
			throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseCollisionMapNode(): <Invalid Node Name %s>", pXmlElement->Value() );
		}
	}
...
void ParseUserMapNode( const TiXmlElement *pelem, EntitySkin & skin, bool bgroup )
{
	...
	for(const TiXmlElement *pXmlElement = pelem->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
	{
		...
		{
			throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "ParseUserMapNode(): <Invalid Node Name %s>", pXmlElement->Value() );
		}
	}
...
bool EntSkinAssetFilterInterface::ConvertAsset( const std::string & FromFileName, const std::string & ToFileName )
{
	...
	try
	{
		...
		else
		{
			...
			{
				throw SkinFilterErr( "<Check AssetName attr on the SkinNode> < %s not found in %s >", AssetNameStr.c_str(), FromFileName.c_str() ); 
			}
			else
			{
				...
				{
					throw SkinFilterErr( "<Check AssetName attr on the SkinNode> < %s != %s >", nextchk.c_str(), AssetNameStr.c_str() ); 
				}
			}
			...
			for(const TiXmlElement *pXmlElement = SkinNode->FirstChildElement(); pXmlElement; pXmlElement = pXmlElement->NextSiblingElement())
			{
				...
				{
					throw SkinFilterErr( pXmlElement->Row(), pXmlElement->Column(), "<Invalid Node Name %s> <LINE %s>", pXmlElement->Value(), __LINE__ );
				}
			}
