      ...
      for (size_t im = 0; im < inumvtxinfs; ++im) {
        ...
        switch (policy->mSkinPolicy.mWeighting) {
          case ColladaSkinPolicy::EPOLICY_MATRIXPALETTESKIN_W4: {
            if (inumpairs > 4) {
              orkerrorlog(
                  "ERROR: <%s> vtxinf<%d> NumWeights<%d> < skinning (no more than 4 allowed), I will attempt to prune "
                  "automagically >\n",
          ...
          case ColladaSkinPolicy::EPOLICY_MATRIXPALETTESKIN_W1: {
            if (inumpairs > 1) {
              orkprintf(
                  "WARNING: <%s> vtxinf<%d> NumWeights<%d> < skinning (no more than 1 allowed), I will attempt to prune "
                  "automagically >: ",
                  policy->mColladaOutName.c_str(),
                  im,
                  inumpairs);
        ...
        if (policy->mSkinPolicy.mWeighting == ColladaSkinPolicy::EPOLICY_MATRIXPALETTESKIN_W1) {
          if (largestWeightMap.size() == 1) {
            ...
            orkprintf("\n");
            orkerrorlog(
                "ERROR: <%s> vertex<%d> numpairs<%d> largestWeightMap<%d>\n",
                policy->mColladaOutName.c_str(),
                im,
                inumpairs,
                largestWeightMap.size());
            orkerrorlog(
                "ERROR: <%s> cannot prune weight, out of tolerance. You must prune it manually\n", policy->mColladaOutName.c_str());
            return false;
          }
        }
        icount             = 0;
        ...
        if (fwtest >= 0.02f) {
          orkerrorlog(
              "ERROR: <%s> vertex<%d> fwtest<%f> numpairs<%d> largestWeightMap<%d>\n",
              policy->mColladaOutName.c_str(),
              im,
              fwtest,
              inumpairs,
  ...
  if (GeoLib) {
    ...
    for (orkmap<std::string, ColMeshRec*>::iterator it = mMeshIdMap.begin(); it != mMeshIdMap.end(); it++) {
      ...
      for (size_t imatgroup = 0; imatgroup < inummatgroups; imatgroup++) {
          ...
          if (pminiorktek) {
            ...
            if (prootnode) {
              ...
              if (pvtxlitnode) {
                ...
                ork::file::Path PngPath = LightMapPath;
                PngPath.SetExtension("png");
        ...
        auto GetFaceCount = [&]() -> size_t { return facvtx_counts.size() - hole_faces.size(); };
        ...
        auto GetFaceVertexCount = [&](size_t index) -> size_t {
          ...
          if (index < GetFaceCount()) {
            ...
            UInt32List::const_iterator end =
                it + holeCount + 1; // +1 in order to sum the face-vertex pairs of the polygon as its holes.
            for (; it != end; ++it)
              count += (*it);
          }
          return count;
        ...
        for (size_t iface = 0; iface < imatnumfaces; iface++) {
          ...
          for (size_t iface_v = 0; iface_v < iface_numfverts; iface_v++) {
            ...
            {
              orkerrorlog(
                  "ERROR: <ColladaModel %s> (Normal Index out of range [nidx %d] [numnormals %d])\n",
                  this->mFileName.c_str(),
                  (inrmidx / 3),
                  (NrmSource->GetDataSize() / 3));
              return false;
            }
