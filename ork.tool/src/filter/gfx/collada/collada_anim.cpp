...
bool CColladaAnim::Parse(void) {
  ...
  for (int ie = 0; ie < inument; ie++) {
    ...
    if (itstrmtx != std::string::npos) {
      ...
      for (int ich = 0; ich < num_child; ich++) {
        auto chld             = Anim->GetChild(ich);
        std::string ChildName = chld->GetName().c_str();
        std::string id        = chld->GetDaeId().c_str();

        if (ChildName == AnimName) {
          // Armature_ArmatureAction_Bone_001_pose_matrix
          // ->
          // Bone.001
          std::string search = AnimName + "_" + AnimName + "Action_";
          ChildName          = ork::string::replaced(id, search, "");
          ChildName          = ork::string::replaced(ChildName, "_pose_matrix", "");
          ChildName          = ork::string::replaced(ChildName, "_", ".");
        }

        size_t num_cchans = chld->GetChannelCount();
        printf("  Child<%d> Name<%s> id<%s> numchans<%d>\n", ich, ChildName.c_str(), id.c_str(), num_cchans);
        for (int icha = 0; icha < num_cchans; icha++) {
          ...
          if (numcrv == 16)
            ValidMatrixAnimSet[ChildName] = chan;
        }
      }
    ...
    else if( it_ma != mFxAnimatables.end() ) // channel name found in FxAnimatables ?
    {
        ...
        switch( param->GetRecord().meParameterType )
        {
            case ork::EPROPTYPE_REAL:
            {
                ...
                OrkAssert( inumcurves==1 );
                FCDAnimationCurve *InputCurve = InputChannel->GetCurve(0);
                ColladaFxAnimChannel<float>* OutputAnimChannel = new ColladaFxAnimChannel<float>( AnimName, AnimBaseName,
    AnimCompName ); mAnimationChannels[ AnimName ] = OutputAnimChannel; int inumkeys = InputCurve->GetKeyCount(); float ffirstkey =
    InputCurve->GetKey(0)->input; float flastkey = InputCurve->GetKey( inumkeys-1 )->input; int iframe = 0; for( float fi=ffirstkey;
    fi<=flastkey; fi+=kfsampleincrement, iframe++ )
                {
                    float KeyFrame = InputCurve->Evaluate( fi );
            ...
            case ork::EPROPTYPE_VEC3FLOAT:
            {
                ...
                ColladaFxAnimChannel<fvec3>* OutputAnimChannel = new ColladaFxAnimChannel<fvec3>( AnimName, AnimBaseName,
    AnimCompName ); mAnimationChannels[ AnimName ] = OutputAnimChannel; FCDAnimationCurve *Curve0 = InputChannel->GetCurve(0); int
    inumkeys = Curve0->GetKeyCount(); float ffirstkey = Curve0->GetKey(0)->input; float flastkey = Curve0->GetKey( inumkeys-1
    )->input; int iframe = 0; FCDAnimationCurve *CurveX = InputChannel->GetCurve(0); FCDAnimationCurve *CurveY =
    InputChannel->GetCurve(1); FCDAnimationCurve *CurveZ = InputChannel->GetCurve(2); for( float fi=ffirstkey; fi<=flastkey;
    fi+=kfsampleincrement, iframe++ )
                {
  ...
  for (auto& ITEM : ValidMatrixAnimSet) {
    ...
    for (int icu = 0; icu < inumcurves; icu++) {
      ...
      printf("chan<%s> lo<%f> hi<%f> inumkeys<%d>\n", MtxName.c_str(), ffirstkey, flastkey, inumkeys);
    }
  }
  ...
  if (mAnimationChannels.size()) {
    ...
    for (orkmap<std::string, ColladaAnimChannel*>::const_iterator it = mAnimationChannels.begin(); it != mAnimationChannels.end();
         it++) {
