...
void BakeJobThread::ActivateJob() {
  ...
  ork::file::Path dst_imagepath = FileEnv::DoesDirectoryExist(dst_imgpathA) ? dst_imgpathA : dst_imgpathB;
  ...
  //////////////////////////////////////////////
  std::string fullcmd_compre =
      CreateFormattedString("%sext\\miniork\\ext\\bin\\bzip2.exe -k -f %s", WorkingFolderPath.c_str(), dst_rgmfilename.c_str());
  //////////////////////////////////////////////
  ...
  //////////////////////////////////////////////
  std::string copythere_options = CreateFormattedString(
  ...
  //////////////////////////////////////////////
  std::string remotedecomp_options = CreateFormattedString(
  ...
  //////////////////////////////////////////////
  std::string remotegelato_options = CreateFormattedString(
      "-v -ssh -C -P %d -i %s michael@%s gelato %s", isshport, id_dsafile.c_str(), machinename.c_str(), dst_rgmfilename.c_str());
  std::string fullcmd_gel = plnkcmd + remotegelato_options;
  //////////////////////////////////////////////
  std::string copyback_options = CreateFormattedString(
  ...
  //////////////////////////////////////////////
  if (blocal) {
    fullcmd_gel = CreateFormattedString(
...
bool PerformBake(BakeOps* pOPS, const BakerSettings* psetting, const FarmNodeGroup* pfarmgroup) {
  ...
  if (jobset) { //////////////////////////////////////////
    ...
    //////////////////////////////////////////
    {
      ...
      for (orklut<PoolString, FarmJob*>::const_iterator it = jobs.begin(); it != jobs.end(); it++) {
  ...
  FarmNodePool& fnpool                       = bkctx->mLockedFarmNodePool.LockForWrite();
  {
    ...
    int idx = 0;
    for (orklut<PoolString, FarmNode*>::const_iterator it = Nodes.begin(); it != Nodes.end(); it++) {
      ...
      bool bcon = bjt->connect(
          bjt, SIGNAL(SigAppendString(QString)), fnpool.mPool.direct_access(idx).mProcessViewer, SLOT(AppendString(QString)));
...
void CollectLights(MeshUtil::LightContainer& lc, const SceneData* psd, const std::string& match) {
  ...
  for (orkmap<PoolString, SceneObject*>::const_iterator itso = scene_objects.begin(); itso != scene_objects.end(); itso++) {
    ...
    if (pentd) {
      ...
      ////////////////////////////////////////////////
      for (tokenlist::const_iterator itm = match_toklist.begin(); itm != match_toklist.end(); itm++) {
        ...
        if (plightdatacomp) {
          if (false == plightdatacomp->isDynamic()) {
            ...
            if (pgl) {
              ...
              pgl->mShadowSamples   = plightdata->shadowSamples();
