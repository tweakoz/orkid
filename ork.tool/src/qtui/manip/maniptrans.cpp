...
void ManipTrans::HandleMouseDown(const ork::fvec2& pos) {
  ...
  if (pcam) {
    mManager.mManipHandler.Init(pos, pcam->_curMatrices.GetIVPMatrix(), pcam->QuatC);
...
void ManipSingleTrans::DrawAxis(Context* pTARG) const {
  auto fbi = pTARG->FBI();
  auto fxi = pTARG->FXI();
  auto gbi = pTARG->GBI();
  ...
  mManager.materialEnd(pTARG);
}

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

void ManipSingleTrans::Draw(Context* pTARG) const {
  fmtx4 Mat;
  fmtx4 VisMat;
  fmtx4 MatT;
  fmtx4 MatS;
...
void ManipDualTrans::Draw(Context* pTARG) const {
  fmtx4 MatCur;
  fvec3 pos;
  fquat rot;
  float scale;
...
void ManipDualTrans::HandleDrag(const ork::fvec2& pos) {
  ...
  {
      mManager.mCurTransform.Translate(TransformNode3D::EMODE_ABSOLUTE, mManager.mCurTransform.GetTransform()->GetPosition() +
  worlddiff); mManager.ApplyTransform(mManager.mCurTransform);
  }
  */
}
