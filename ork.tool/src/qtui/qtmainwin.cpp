...
void MiniorkMainWindow::UpdateTitle() {
  QString title = mMainTitle + QString(" - ") + mMiniorkRepos + QString(":") + mMiniorkRev + QString(" - ") + mGameRepos +
                  QString(":") + mGameRepos;
...
void EditorModule::AddAction(QMenu* pmenu, const char* pname) {
  ...
  printf("action<%s:%p>\n", pname, pmenu);
  assert(false);
  QObject::connect(action, &QAction::triggered, this, &EditorModule::ActionSlot);
}
///////////////////////////////////////////////////////////////////////////////
...
void EditorModule::AddAction(const char* pname, QKeySequence ks) {
  auto pnode = mSlashHier.add_node(pname, 0);
    ...
    if (node != mSlashHier.root().get()) {
      ...
      if (nm != "/") {
        ...
        if (end) {
          ...
          QObject::connect(pact, &QAction::triggered, this, &EditorModule::ActionSlot);
        } else {
          if (0 == pcurmenu) {
            pcurmenu = (0 == pdata) ? mMenuBar->addMenu(nm.c_str()) : (QMenu*)pdata;
          } else {
            pcurmenu = (0 == pdata) ? pcurmenu->addMenu(nm.c_str()) : (QMenu*)pdata;
          }
          if (0 == pdata) {
