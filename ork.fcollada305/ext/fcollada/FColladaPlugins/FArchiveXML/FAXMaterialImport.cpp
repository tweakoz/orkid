...
bool FArchiveXML::LoadEffectParameterSurface(FCDObject* object, xmlNode* parameterNode)
{
	...
	for (xmlNodeList::iterator it = valueNodes.begin(); it != valueNodes.end(); ++it)
	{
		...
		if (effectParameterSurface->GetNames().size() == 0 || effectParameterSurface->GetNames()[0].empty())
		{
			...
			FUError::Error(FUError::WARNING_LEVEL, FUError::WARNING_EMPTY_INIT_FROM, parameterNode->line);
		}
	...
	if (formatHintNode)
	{
		FCDFormatHint* formatHint = new FCDFormatHint();
		effectParameterSurface->SetFormatHint(formatHint);
...
bool FArchiveXML::LoadEffectPassShader(FCDObject* object, xmlNode* shaderNode)	
{
	...
	bool isFragment = (stage == DAE_FXCMN_FRAGMENT_SHADER);
	if (isFragment)
		effectPassShader->AffectsFragments();
	else
		effectPassShader->AffectsVertices();
	if (!effectPassShader->IsFragmentShader() && !IsEquivalent(stage, DAE_FXCMN_VERTEX_SHADER))
	{
...
bool FArchiveXML::LoadImage(FCDObject* object, xmlNode* imageNode)
{
	...
	fstring tofstring = ReadNodeContentFull(filenameSourceNode);
	tofstring = FixFuxedUpColladaPaths( tofstring );

	image->SetFilename(tofstring); //TO_FSTRING());

	// Convert the filename to something the OS can use
	const fstring& imgfname = image->GetFilename();
	//const char* pfname = imgfname.c_str();
	//printf( "pfname<%s>\n", pfname );
	FUUri val = imgfname;
	fstring fileName = val.GetAbsoluteUri();
