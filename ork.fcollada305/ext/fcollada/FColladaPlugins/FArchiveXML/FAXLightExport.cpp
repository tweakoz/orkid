...
xmlNode* FArchiveXML::WriteLight(FCDObject* object, xmlNode* parentNode)
{
	...
	else if (light->GetLightType() == FCDLight::DIRECTIONAL)
	{
		...
		attenuationNode->GetAnimated()->Clone(light->GetConstantAttenuationFactor().GetAnimated());
		...
		attenuationNode->GetAnimated()->Clone(light->GetLinearAttenuationFactor().GetAnimated());
		extraParameterNodes.push_back(attenuationNode);
		attenuationNode = techniqueNode->AddParameter(DAE_QUAD_ATTENUATION_LIGHT_PARAMETER, light->GetQuadraticAttenuationFactor());
		attenuationNode->GetAnimated()->Clone(light->GetQuadraticAttenuationFactor().GetAnimated());
		extraParameterNodes.push_back(attenuationNode);
	}
	...
	else if (light->GetLightType() == FCDLight::DIRECTIONAL)
	{
		...
		falloffNode->GetAnimated()->Clone(light->GetFallOffAngle().GetAnimated());
		...
		falloffNode->GetAnimated()->Clone(light->GetFallOffExponent().GetAnimated());
		extraParameterNodes.push_back(falloffNode);
	}

	FCDENode* intensityNode = techniqueNode->AddParameter(DAEFC_INTENSITY_LIGHT_PARAMETER, light->GetIntensity());
	intensityNode->GetAnimated()->Clone(light->GetIntensity().GetAnimated());
	...
	if (light->GetLightType() == FCDLight::DIRECTIONAL || light->GetLightType() == FCDLight::SPOT)
	{
		...
		outerAngleNode->GetAnimated()->Clone(light->GetOuterAngle().GetAnimated());
	...
	if (light->GetLightType() == FCDLight::SPOT)
	{
		...
		dropoffNode->GetAnimated()->Clone(light->GetDropoff().GetAnimated());
