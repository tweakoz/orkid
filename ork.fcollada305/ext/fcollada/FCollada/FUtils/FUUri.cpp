		...
		path(FC(""))
{
	...
	{
		//printf( "append / a\n" );
		if( *(path.c_str()+(path.length()-1)) != FC('/') )
		{
			path.append(FC("/"));
		}
	}
...
{
	fstring _relativePath = FixFuxedUpColladaPaths( relativePath );

	if (_relativePath.empty()) return _relativePath;
	FUUri uri(_relativePath);
...
bool FUUri::IsFile() const
{
	...
	fchar chend = (ilen>1) ? (*(path.c_str()+(ilen-1))) : 0;

	bool isfile = ilen > 1 && chend != '/';

	if( ilen>1 && false == isfile )
	{
		//printf( "! a file <%s> ilen<%d> chend<%c>\n", path.c_str(), ilen, chend );
	}
...
fstring FUUri::Escape(const fstring path)
{
	...
	for (fstring::const_iterator it = path.begin(); it != path.end(); ++it)
	{
		...
		fstring _escaped = TO_FSTRING(FUStringConversion::ToString(int32(*it)));
