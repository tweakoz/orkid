...
namespace fm
{
	...
	public:
		...
		void resize(size_t size, const CH& value)
		{
...
/*class twkstring : public std::string
{
public:
		
	inline operator const fchar*() const { return (const fchar*)c_str(); } // see above
	twkstring(const fchar* c) //: Parent()
	{
		append(c);
		if (c == NULL || (*c) == 0) push_back((fchar) 0);
	}
	twkstring() {}

};*/

//typedef twkstring fstring;
