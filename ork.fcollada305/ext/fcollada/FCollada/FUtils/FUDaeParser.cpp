...
namespace FUDaeParser
{
	...
	// Retrieves a series of interpolation values from a source node
	void ReadSourceInterpolationInterleaved(xmlNode* sourceNode, fm::pvector<UInt32List>& arrays)
	{
		if (sourceNode != NULL)
		{
			// Get the accessor's count
			xmlNode* accessorNode = FindTechniqueAccessor(sourceNode);
			uint32 count = ReadNodeCount(accessorNode);
			for (fm::pvector<UInt32List>::iterator it = arrays.begin(); it != arrays.end(); ++it)
			{
				(*it)->resize(count);
			}
	...
	void SplitTarget(const fm::string& target, fm::string& pointer, fm::string& qualifier)
	{
	...
	int32 ReadTargetMatrixElement(fm::string& qualifier)
	{
		int32 returnValue = -1;
	...
	FUUri ReadNodeUrl(xmlNode* node, const char* attribute)
	{
		...
		size_t suffixStart = uriString.find('#');
		if (suffixStart == fm::string::npos) out.prefix = TO_FSTRING(uriString);
		else
		{
