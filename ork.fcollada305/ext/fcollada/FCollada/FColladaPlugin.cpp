...
FCPArchive* FColladaPluginManager::FindArchivePlugin(const fchar* filename)
{
	...
	FUUri fileUri(ffilename);
	fstring fpath = fileUri.GetPath();

	fstring extension = FUFileManager::GetFileExtension(fpath);
	...
	printf( "file name<%s> path<%s>\n", filename, fpath.c_str() );

	for (size_t i = 0; i < archivePlugins.size(); ++i)
	{
		FCPArchive* curArchive = archivePlugins[i];
		printf( "curArchive<%d><%p>\n", (int)i, curArchive );
		for (int j = 0; j < curArchive->GetSupportedExtensionsCount(); ++j)
		{
			fstring targetExt = FUStringConversion::ToFString(curArchive->GetSupportedExtensionAt(j));
			fstring mstr = targetExt.c_str();
			fstring mstr2 = extension.c_str();
			bool isequiv = IsEquivalentI(extension, targetExt);
			printf( "isequiv<%d:%d>, ext<%s> targetExt<%s>\n", j, int(isequiv), mstr2.c_str(), mstr.c_str() );
		}
	}
	
	fstring mstr = filename;

	printf( "Num Archivers<%d> filename<%s>\n", int(archivePlugins.size()), mstr.c_str() );
