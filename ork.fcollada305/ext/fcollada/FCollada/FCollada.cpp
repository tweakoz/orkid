...
#include "../FColladaPlugins/FArchiveXML/StdAfx.h"
#include "../FColladaPlugins/FArchiveXML/FArchiveXML.h"
...
#if 0 //def WIN32
	extern __declspec(dllexport) FUPlugin* CreatePlugin(uint32);
#else 
//	FCOLLADA_EXPORT
	//FUPlugin* CreatePlugin(uint32) { return new FArchiveXML(); }
#endif // WIN32
...
namespace FCollada
{
	...
	FCOLLADA_EXPORT void Initialize()
	{
		if (pluginManager == NULL)
		{
			...
			pluginManager->RegisterPlugin(new FArchiveXML());
		}
	...
	FCOLLADA_EXPORT bool LoadDocumentFromFile(FCDocument* document, const fchar* filename)
	{
		...
		//printf( "fcolladapluginmanager<%08x>\n", pluginManager );
		int irand = rand();
		char* yo = new char[irand];
		delete[] yo;

		FUAssert(pluginManager != NULL, return false);
		bool bv = pluginManager->LoadDocumentFromFile(document, filename);
