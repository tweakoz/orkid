...
public:
	...
	inline void SetAnimationName(fm::string name, size_t index) { if (names.size() <= index) names.resize(index + 1); names[index] = name; }
	inline fm::string GetAnimationName(size_t i) const { return names[i]; }
