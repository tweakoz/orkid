...
public:
	typedef ork::orklut<ork::PoolString, ork::lev2::XgmAnimAsset *> AnimationMap;
	...
	const AnimSeqTableMap &GetAnimSeqTableMap() const { return mAnimSeqTableMap; }
	AnimSeqTableMap &GetAnimSeqTableMap() { return mAnimSeqTableMap; }

	const AnimMaskMap &GetAnimMaskMap() const { return mAnimMaskMap; }
	AnimMaskMap &GetAnimMaskMap() { return mAnimMaskMap; }
...
{
	RttiDeclareAbstract(SimpleAnimatableInst, ork::ent::ComponentInst)
public:
	...
	void PlayAnimationEx(ork::PoolString maskname, ork::PoolString name, int priority = 0, float speed = 1.0f, float interp_duration = 0.0f, bool loop = true);
	...
	void SetModelInst(ork::lev2::XgmModelInst *inst) { mModelInst = inst; }
	const SimpleAnimatableData &GetData() {return mData;}
...
private:
	...
	ork::lev2::XgmModelInst *GetModelInst() const { return mModelInst; }

	struct AnimationQueueItem
	{
		AnimationQueueItem(ork::PoolString name = ork::AddPooledLiteral(""), float speed = 1.0f, float interp_duration = 0.0f, bool loop = true)
			: mName(name), mSpeed(speed), mInterpDuration(interp_duration), mLoop(loop) {}
	...
	public:
		...
		AnimData(const SimpleAnimatableInst &sai) : mSai(sai), mSpeed(1.0f), mAnimSeqTable(NULL) {}
		...
		const ork::lev2::XgmAnimInst &AnimInst() const { return mAnimInst; }

		void BindAnim(const ork::lev2::XgmAnim *anim);
		void SetFrame(float f) { mAnimInst.SetCurrentFrame(f); }
		float GetFrame() { return mAnimInst.GetCurrentFrame(); }
		void SetWeight(float w) { mAnimInst.SetWeight(w); }
		float GetWeight() { return mAnimInst.GetWeight(); }
		void SetAnimSeqTable(AnimSeqTable *table) { mAnimSeqTable = table; }
		ork::lev2::XgmAnimMask &RefMask() { return mAnimInst.RefMask(); }
	};

	struct AnimBodyPart
	{
		AnimBodyPart(const SimpleAnimatableInst &sai) : mCurrentAnimData(sai), mPreviousAnimData(sai), mInterpSpeed(0.0f), mPriority(0) {}
	...
	void PlayAnimationOnMask(BodyPartMap::iterator &itmask, SimpleAnimatableData::AnimationMap::const_iterator &itanim,
