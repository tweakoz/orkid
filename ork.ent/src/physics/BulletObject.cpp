...
void BulletObjectControllerData::Describe() {
  //	reflect::RegisterProperty( "Model", & BulletObjectControllerData::GetModelAccessor, &
  //BulletObjectControllerData::SetModelAccessor );
  ...
  reflect::annotatePropertyForEditor<BulletObjectControllerData>(
  ...
  reflect::RegisterProperty("Shape", &BulletObjectControllerData::ShapeGetter, &BulletObjectControllerData::ShapeSetter);
...
BulletObjectControllerInst::BulletObjectControllerInst(const BulletObjectControllerData& data, ork::ent::Entity* pent)
  ...
  for (orkmap<PoolString, ork::Object*>::const_iterator it = forcecontrollers.begin(); it != forcecontrollers.end(); it++) {
...
bool BulletObjectControllerInst::DoLink(Simulation* psi) {
  ...
  if (auto bulletsys = psi->findSystem<BulletSystem>()) {
    ...
    if (btDynamicsWorld* world = bulletsys->GetDynamicsWorld()) {
      ...
      for (orkmap<PoolString, BulletObjectForceControllerInst*>::const_iterator it = mForceControllerInstMap.begin();
