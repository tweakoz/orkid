...
struct BlockMemImpl : public Module {
  BlockMemImpl(std::string name, Module* parent, args_t args, Ref storage, size_t width, size_t depth);
};
...
BlockMemPort::BlockMemPort(std::string name, Module* m, size_t w, size_t d)
    : _width(w)
...
void BlockMemStorage::initFromData(const char* data, size_t length) {
  ...
  if (length > _depth) {
    printf("BlockMemStorage<%p> initdata length<%zu> truncated to depth<%zu>", (void*)this, length, _depth);
    length = _depth;
  }
...
struct MemInit : public AstNode {
  MemInit(std::string nam, BlockMemStorage* s)
      : _modulename(nam)
      , _storage(s) {
  }
...
struct BlockMem : public Module {
  BlockMem(std::string name, Module* parent, args_t args, size_t width, size_t depth);
...
BlockMem::BlockMem(std::string name, Module* parent, args_t args, size_t width, size_t depth)
...
void BlockMem::generate() { // final
  ...
  auto s = _storage->_storage.get();
  hdl_sync({
    hdl_if(_port->write_enable, { (*s)[_port->address].next(_port->datain); });
    hdl_else({ _port->dataout.next((*s)[_port->address]); });
  });
}
...
DualPortBlockMem::DualPortBlockMem(std::string name, Module* parent, args_t args)
    : Module(name, parent, {})
    , _args(args) {
...
void DualPortBlockMem::generate() { // final
  ...
  auto s = _storage->_storage.get();
  hdl_sync({
    hdl_if(_portA->write_enable, { (*s)[_portA->address].next(_portA->datain); });
    hdl_else({ _portA->dataout.next((*s)[_portA->address]); });
  });
}
