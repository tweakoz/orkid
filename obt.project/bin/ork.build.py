...
#  print("BUILD_DEST: %s" % build_dest)
  ...
  # that link phase sharply. Passed as -D cache vars (not the LDFLAGS
  # env-var route pytorch needed) because this is a direct cmake
  ...
  if rval==0 and obt.host.IsDarwin:
    # NOTE: orkid links the real libtorch_* directly from $OBT_PYPKG/torch/
    # lib (see orkid.cmake ork_torch_opts) — no torch dylibs are copied
    # into stage/lib, so the macho fixup below never touches torch.
    #
