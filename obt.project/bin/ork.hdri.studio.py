...
fragment_shader fs_prev : iface_fprev {
  vec2 uv = frg_uv0.xy * uv_scale + uv_offset;
  vec4 src = texture(ColorMap, uv);
  vec3 c = src.rgb;
  c *= gain;
  ...
  // Fade to black outside [0,1] UV range when zoomed in
  float border = step(0.0, uv.x) * step(uv.x, 1.0) * step(0.0, uv.y) * step(uv.y, 1.0);
  out_color = vec4(c * border, 1.0);
}
...
class EnvMapStudio(ComponentizedApplication):
  ...
  def __init__(self):
    ...
    # Zoom/pan state for image viewers
    self._zoom = 1.0          # 1.0 = fit, >1 = zoomed in
    self._pan = [0.0, 0.0]    # UV offset (center-based)
    self._drag_last = None    # last mouse pos during drag
  ...
  def _onUiInit(self):
    ...
    self.source_imv.evhandler = lambda ev: self._on_viewer_event(ev)
  ...
  def _uv_offset(self):
    """Compute UV offset for shader: pan is in UV space, centered."""
    return vec2(self._pan[0], self._pan[1])

  def _uv_scale(self):
    """Compute UV scale for shader: 1/zoom."""
    s = 1.0 / max(self._zoom, 0.01)
    return vec2(s, s)
  ...
  def _on_viewer_event(self, ev):
    """Handle zoom (scroll) and pan (left-drag) on image viewers."""
    # unit_pos is window-relative 0-1, used for zoom-to-cursor
    ux, uy = ev.unit_pos.x, ev.unit_pos.y

    code = ev.code
    if code == tokens.MOUSEWHEEL.hashed:
      old_zoom = self._zoom
      factor = 1.01 if ev.wheel_y > 0 else (1.0 / 1.01)
      self._zoom = max(0.1, min(self._zoom * factor, 100.0))

      old_scale = 1.0 / old_zoom
      new_scale = 1.0 / self._zoom
      # UV under mouse before zoom
      mu = self._pan[0] + ux * old_scale
      mv = self._pan[1] + uy * old_scale
      # Adjust pan so same UV stays under mouse
      self._pan[0] = mu - ux * new_scale
      self._pan[1] = mv - uy * new_scale

      result = lev2.ui.HandlerResult()
      return result
    elif code == tokens.PUSH.hashed and (ev.left or ev.middle):
      self._drag_last = (ev.x, ev.y)
      result = lev2.ui.HandlerResult()
      return result
    elif code == tokens.DRAG.hashed and (ev.left or ev.middle) and self._drag_last:
      dx = ev.x - self._drag_last[0]
      dy = ev.y - self._drag_last[1]
      self._drag_last = (ev.x, ev.y)
      # Convert pixel delta to UV delta
      sdim = ev.screen_dim
      if sdim.x > 0 and sdim.y > 0:
        self._pan[0] -= dx / sdim.x / self._zoom
        self._pan[1] -= dy / sdim.y / self._zoom
      result = lev2.ui.HandlerResult()
      return result
      ...
      # Escape to reset zoom/pan (keycode 256)
      if ev.keycode == 256:
  ...
  def _load_source(self, ctx):
    ...
    self.preview_pipeline.bindParam(mtl.param("uv_offset"), lambda: self._uv_offset())
    self.preview_pipeline.bindParam(mtl.param("uv_scale"), lambda: self._uv_scale())

    # Reset zoom/pan for new source
    self._reset_zoom_pan()
  ...
  def _make_display_pipeline(self, ctx, accum_ssbo, img_w, img_h):
    ...
    pipeline.bindParam(mtl.param("uv_offset"), lambda: self._uv_offset())
    pipeline.bindParam(mtl.param("uv_scale"), lambda: self._uv_scale())
   ...
   #print(f"  Creating level {idx+1}/{nlev}: {spec['label']}")
    ...
    if is_diffuse:
      ...
      if spec_idx < len(self._result_tabs):
        ...
        imv.maintain_aspect_ratio = True
        imv.generate_mipmaps = False
        imv.evhandler = lambda ev: self._on_viewer_event(ev)
        dummy = lev2.Image()
        dummy.initWithFormat(iw, ih, tokens.RGBA8)
        imv.setImage(dummy)
