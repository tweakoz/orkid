...
def generate_docs(module_name, output_dir):
    ...
    def replace_outside_quotes(text, target, replacement,chk_apis=False):
        print(f"Replacing {target} with {replacement} outside quotes")
    ...
    doc_html = replace_outside_quotes(doc_html,"orkengine.core._core.","",chk_apis=True)
    doc_html = replace_outside_quotes(doc_html,"orkengine.lev2._lev2.","",chk_apis=True)
    doc_html = replace_outside_quotes(doc_html,str(path.stage()),"<staging>",chk_apis=False)
