...
class Hydro(HeightField):
    def __init__(self, octaves=6, iterations=200,
                 rain=0.75, evaporation=0.015, capacity=0.15, erosion=0.255, deposition=0.50):
        super().__init__()
        base = T.Fbm(frequency=3.0, octaves=octaves) * 0.5 + 0.5
        h0 = T.erode_thermal(base, iterations=150, talus_deg=33.0, rate=0.15)
        h1 = T.erode_hydro(h0, iterations=iterations, rain=rain, evaporation=evaporation,
                          capacity=capacity, erosion=erosion, deposition=deposition)
        h2 = T.erode_hydro(h1, iterations=iterations, rain=rain, evaporation=evaporation,
                          capacity=capacity, erosion=erosion, deposition=deposition)
        self.capture(h2,"height")
