...
class ForestScene(Scene):
  ...
  def __init__(self):
    ...
    self.scenegraph(
        ...
        CullFrustumScale   = 1.1,   # OWNER DEBUG AID (deliberate): narrow cull frustum so culling
                                     # is VISIBLE at view edges — keep until owner says otherwise
        DepthPrepass       = True,   # ON by owner ruling 2026-08-02. Was hardcoded False as a
                                     # stale invisibility-bisection toggle, so forest never ran the
                                     # prepass and was not comparable to swest (which inherits it).
                                     # ORKID_DPP=0/1 overrides this engine-wide (ork.renderoverrides).
        msaa = 2,
        ssaa = 0,
