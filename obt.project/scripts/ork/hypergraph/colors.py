...
#
# Provides:
#   - hsv(h, s, v[, a]) / wavelength(nm[, a]) / colortemp(k[, a]): return a
#     REAL vec3 when alpha is omitted, a REAL vec4 when alpha is given —
#     directly usable at any raw slot (pyext setters, gradient stops, …).
#   - colors: 64-name palette (colors.red, colors.oak1, colors.skyblue1, …)
#     — palette entries stay LAZY (_Hsv) and auto-coerce vec3/vec4 at the
#     consumption slot; .to_vec3() / .to_vec4() materialize explicitly.
#   - mix(a, b, t): lerp; lazy in → lazy out, raw vecs in → raw vec out.
#   - _PBR_VEC3_FIELDS / _PBR_VEC4_FIELDS: registry of which material fields
#     expect vec3 vs vec4 (used by the PbrMaterial dispatcher's coercion)
#
...
def hsv(h, s, v, a=None):
  """HSV(A) → vec3 (alpha omitted) or vec4 (alpha given).

  Hue in degrees [0, 360], saturation/value/alpha in [0, 1]. Returns a REAL
  vec3/vec4 — usable anywhere one is (pyext setters, gradient stops, math).
  Slot-aware consumers (PbrMaterial fields) upgrade vec3 → vec4(rgb, 1) for
  4-component slots, so the alphaless form works everywhere opaque.

  Examples:
      albedo     = hsv(200, 0.2, 0.7)        # → vec3
      base_color = hsv(0,   1.0, 1.0, 0.5)   # → vec4 red half-transparent
      stops      = {0.0: hsv(30, 1, 1, 0.0)} # vec4 gradient stop (additive)"""
  r, g, b = _hsv_to_rgb(h, s, v)
  if a is None:
    return vec3(r, g, b)
  return vec4(r, g, b, float(a))


def wavelength(nm, a=None):
  """Visible-spectrum wavelength (nm) → vec3 (alpha omitted) or vec4 (alpha
  given). Same return contract as hsv().
...
def colortemp(k, a=None):
  """Black-body color temperature (Kelvin) → vec3 (alpha omitted) or vec4
  (alpha given). Same return contract as hsv().
...
def mix(a, b, t=0.5):
  ...
  `a` and `b` may be _LazyColor (palette), vec3/vec4 (hsv() returns), or
  3/4-tuples; `t` is a scalar in [0, 1] (unclamped — extrapolation works).
  Return type follows the inputs: any lazy input → lazy _RgbaColor (still
  auto-coerces at the slot); raw inputs → vec4 if either carries alpha,
  else vec3."""
  def _decompose(c):
    # → (rgba 4-tuple, is_lazy, has_alpha_channel)
    if isinstance(c, _LazyColor):
      return c._rgba, True, True
    if isinstance(c, vec4):
      return (c.x, c.y, c.z, c.w), False, True
    if isinstance(c, vec3):
      return (c.x, c.y, c.z, 1.0), False, False
    tup = tuple(float(v) for v in c)
    if len(tup) == 3:
      return tup + (1.0,), False, False
    return tup, False, True
  ra, lazy_a, alpha_a = _decompose(a)
  rb, lazy_b, alpha_b = _decompose(b)
  t = float(t)
  out = tuple((1.0 - t) * x + t * y for x, y in zip(ra, rb))
  if lazy_a or lazy_b:
    return _RgbaColor(out)
