...
def export_character_mesh(blend_path, glb_path):
  ...
  for obj in bpy.context.view_layer.objects:
    visible_mesh = (obj.visible_get() and obj.type == 'MESH'
                    )  # Check if the object is visible and is a mesh
    ...
    if visible_mesh and obj.name not in visited:
      ...
      if obj.name in bpy.context.scene.collection.objects:
        ...
        bpy.context.scene.collection.objects.unlink(
            obj)  # Unlink object from the original collection
  ...
  export_options = {
      ...
      "export_attributes":
      True,  # 'Export Attributes (when starting with underscore)',
      ##################################################
  ...
  export_options2 = {
      ...
      False,  # 'Use Current Frame as Object Rest Transformations'
