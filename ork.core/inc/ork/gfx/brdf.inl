...
dvec3 rcp(const dvec3& inp){
  return dvec3(1.0/inp.x,1.0/inp.y,1.0/inp.z);
}

double saturate( double inp ){
  return (inp>1.0)
         ? 1.0
         : ((inp<0.0)
           ? 0.0
...
dvec2 hammersley(uint32_t index, uint32_t sampleCount)
{
...
dvec3 hemisphereSample_uniform(double u, double v) {
  double phi = v*PI2;
...
dvec3 hemisphereSample_cos(double u, double v) {
  double phi = v*PI2;
...
dvec3 sphericalToCartesian(double PhiAngle, double CosTheta, double SinTheta){
  return dvec3(SinTheta * cos(PhiAngle),
               SinTheta * sin(PhiAngle),
               CosTheta);
}

dvec3 importanceSampleBlinn(dvec2 e, double roughness)
{
...
dvec3 importanceSampleGGX(dvec2 e, float roughness)
{
    const double rufsq = roughness*roughness;
...
double geometrySchlickGGX(dvec3 normal, dvec3 dir, double roughness){
...
double geometrySmith(dvec3 normal, dvec3 viewdir, dvec3 lightdir, double roughness){
  return geometrySchlickGGX(normal, viewdir, roughness)
       * geometrySchlickGGX(normal, lightdir, roughness);
}

dvec3 fresnelSchlick(dvec3 normal, dvec3 viewdir, dvec3 F0){
  using namespace std;
...
dvec2 integrateGGX(double n_dot_v, double roughness) {
