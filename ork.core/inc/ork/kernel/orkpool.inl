...
public:
  ...
  const orkvector<pointer>& used() const {
    return m_used;
  }

  value_type& direct_access(int idx) {
    return m_data[idx];
  }
  const value_type& direct_access(int idx) const {
    return m_data[idx];
  }
 ...
 */
template <typename T> class allocator_pooled {
public:
  // typedef Data& reference;
  // typedef const Data& const_reference;
  // typedef Data value_type;
  ...
  template <class OtherData> struct rebind {
  ...
  template <class U> allocator_pooled(const allocator_pooled<U>&) /*throw()*/ {
  }

  allocator_pooled(size_t size)
      : m_pool(size) {
  }
...
template <typename Data, size_t ksize> class fixed_pool {
public:
  typedef Data& reference;
  typedef const Data& const_reference;
  typedef Data value_type;
  ...
  const fixedvector<pointer, ksize>& used() const {
    return m_used;
  }
  fixedvector<pointer, ksize>& used() {
    return m_used;
  }

  value_type& direct_access(size_t idx) {
    return m_data[idx];
  }
  const value_type& direct_access(size_t idx) const {
    return m_data[idx];
  }
...
template <typename pool_adapter_t> //
ObjectPoolX<pool_adapter_t>::item_t ObjectPoolX<pool_adapter_t>::borrowItem() {
  if (_freeItems.empty()) {
    for (size_t i = 0;                             //
...
template <typename pool_adapter_t, size_t KMAX> //
struct BoundedConcurrentObjectPoolX {
  using item_t = typename pool_adapter_t::item_t;

  BoundedConcurrentObjectPoolX(pool_adapter_t tr);
  ~BoundedConcurrentObjectPoolX();
  item_t borrowItem();
  void returnItem(item_t item);

  pool_adapter_t _config;
  MpMcBoundedQueue<item_t, KMAX> _freeItems;
  using retained_items_t = std::vector<item_t>;
  LockedResource<retained_items_t> _retainedItems;
};

///////////////////////////////////////////////////////////////////////////////

template <typename pool_adapter_t, size_t KMAX> //
BoundedConcurrentObjectPoolX<pool_adapter_t, KMAX>::BoundedConcurrentObjectPoolX(pool_adapter_t tr)
    : _config(tr) {
}

template <typename pool_adapter_t, size_t KMAX> //
BoundedConcurrentObjectPoolX<pool_adapter_t, KMAX>::~BoundedConcurrentObjectPoolX() {
  _retainedItems.atomicOp([&](retained_items_t& retained) { retained.clear(); });
}

template <typename pool_adapter_t, size_t KMAX> //
BoundedConcurrentObjectPoolX<pool_adapter_t, KMAX>::item_t BoundedConcurrentObjectPoolX<pool_adapter_t, KMAX>::borrowItem() {
  item_t item;

  // Try to get an item from free queue first
  if (_freeItems.try_pop(item)) {
    return item;
  }

  // No free items available, allocate a batch
  for (size_t i = 0; i < pool_adapter_t::_num_alloc_per_batch; i++) {
    item_t new_item = _config.allocFresh();
    _retainedItems.atomicOp([&](retained_items_t& retained) {
      OrkAssert(retained.size() < KMAX);
      retained.push_back(new_item);
    });

    if (i == 0) {
      // Use the first item as our return value
      item = new_item;
    } else {
      // Put the rest in the free queue
      // This should always succeed since we just allocated
      while (!_freeItems.try_push(new_item)) {
        // Spin until we can push (queue shouldn't be full after allocation)
      }
    }
  }
...
template <typename pool_adapter_t, size_t KMAX> //
void BoundedConcurrentObjectPoolX<pool_adapter_t, KMAX>::returnItem(item_t item) {
  // Must succeed - spin until we can return the item
  while (!_freeItems.try_push(item)) {
    // Spin waiting for space in the queue
  }
}
