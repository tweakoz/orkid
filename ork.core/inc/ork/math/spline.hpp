...
template <typename T>
bool MapU(	float fu, const float fmax, const typename orkmap<float,T>& themap,
			float& flower, float& fupper, float& frange, float& flerp, int& indexl )
{
	bool brv = false;
...
template <typename T>
void CatmullRomSpline<T>::AddCV( const T& cv )
{
...
template <typename T>
void CatmullRomSpline<T>::GetCV( int idx, T& out ) const
{
...
template <typename T>
void CatmullRomSpline<T>::ClearCVS( void )
{
	mSeqVertices.clear();
}
...
template <typename T>
void CatmullRomSpline<T>::GenBases(int pt) const
{
	OrkAssert( pt>=0 );
	OrkAssert( pt<NumCVs() );
...
template <typename T>
void CatmullRomSpline<T>::SampleAt( float fU, T& res, T& deriv ) const
{
...
template <typename T>
int CatmullRomSpline<T>::GetCVIndex( int in ) const
{
...
template <typename T>
int CatmullRomSpline<T>::BaseIndex( int ipoint, int icomponent ) const
{
