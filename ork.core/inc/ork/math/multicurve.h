...
public:
	...
	ObjProxy( T* val ) : mParent(val) {}
};

///////////////////////////////////////////////////////////////////////////////

class  MultiCurve1D : public ork::Object
{
	...
	ork::Object* ProxyAccessor() { return & mProxy; }

public:

	int GetNumSegments() const;
	float Sample( float u ) const;
	const std::pair<float,float>& GetVertex( int iv ) const { return *(mVertices.begin()+iv); }
	EMCSEGTYPE GetSegmentType( int is ) const { return mSegmentTypes[is]; }
	size_t GetNumVertices() const { return mVertices.size(); }
	orklut<float,float>& GetVertices() { return mVertices; }
	...
	void SetMin( float fmin ) { mMin=fmin; }
	void SetMax( float fmax ) { mMax=fmax; }
...
private:
	...
	bool PreDeserialize( ork::reflect::IDeserializer& deser ) override;	// virtual
