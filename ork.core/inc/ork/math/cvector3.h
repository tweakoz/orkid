...
public:
	...
	Vector3( U32 uval ) { SetRGBAU32( uval ); }
	...
	static	Vector3	UnitY(void) { return Vector3(T(0), T(1), T(0)); }
	static	Vector3	UnitZ(void) { return Vector3(T(0), T(0), T(1)); }
	static  Vector3	One(void) { return Vector3(T(1), T(1), T(1)); }
  ...
  void SetRGBAU64( uint64_t v );
	...
	template <typename U>
	static Vector3 FromVector3(Vector3<U> vec)
	{
		return Vector3(T::FromFX(vec.GetX().FXCast()),
...
template <typename T>
inline ork::Vector3<T> operator*( T scalar, const ork::Vector3<T> &b )
{
