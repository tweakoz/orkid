...
inline std::string hexdumptostr(const uint8_t* bytes, size_t length) {
  std::string rval;
  bool keep_going   = length > 0;
  size_t byte_index = 0;
  while (keep_going) {
    ...
    ///////////////////////////////////////////
    size_t bidx0 = byte_index;
    rval += FormatString("0x%02x: ", (unsigned int)byte_index);
    for (int c = 0; c < line_length; c++) {
      uint8_t byte = bytes[bidx0++];
      rval += FormatString("%02x ", byte);
    }
    for (int c = line_length; c < 16; c++) {
      rval += "-- ";
    }
