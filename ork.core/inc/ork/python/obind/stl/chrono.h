/*
    ork/python/obind/stl/chrono.h: conversion between std::chrono and python's datetime
...
#include <ork/python/obind/stl/detail/chrono.h>
...
inline std::tm *localtime_thread_safe(const Time *time, Buf *buf) {
    if constexpr (decltype(can_localtime_s(time, buf))::value) {
        ...
        static_assert(decltype(can_localtime_r(time, buf))::value,
                      "<ork/python/obind/stl/chrono.h> type caster requires "
