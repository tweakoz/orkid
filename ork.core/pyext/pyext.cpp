...
void coreappinit() {
  ...
  static auto WorkingDirContext = std::make_shared<ork::FileDevContext>();
  auto base_dir                 = ork::file::GetStartupDirectory();
  if (getenv("ORKID_WORKSPACE_DIR") != nullptr)
    base_dir = getenv("ORKID_WORKSPACE_DIR");
  ork::OldSchool::SetGlobalStringVariable("data://", base_dir.c_str());
  printf("ORKID_WORKSPACE_DIR<%s>\n", base_dir.c_str());

  ork::rtti::Class::InitializeClasses();
}

ork::PoolString addpoolstring(std::string str) {
  return ork::AddPooledString(str.c_str());
}
...
PYBIND11_MODULE(orkcore, m) {
  ...
  /////////////////////////////////////////////////////////////////////////////////
  m.def("coreappinit", &coreappinit);
  m.def("addpoolstring", &addpoolstring);
  /////////////////////////////////////////////////////////////////////////////////
  py::class_<PoolString>(m, "PoolString").def("__repr__", [](const PoolString& s) -> std::string {
    fxstring<64> fxs;
    fxs.format("PoolString(%s)", s.c_str());
    return fxs.c_str();
  });
  /////////////////////////////////////////////////////////////////////////////////
  py::class_<file::Path>(m, "Path")
