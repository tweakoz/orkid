...
try:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # We need to directly modify the LocationInfo since there's no API for this yet
        ...
        print(f"  Encrypted dir: {cat.encrypted_dir}")
        print(f"  Chunks dir: {cat.chunks_dir}")
        print(f"  Receipts dir: {cat.receipts_dir}")
        ...
        encrypted_file = os.path.join(cat.encrypted_dir, f"{asset.storage_hash}.enc")
        print(f"  Looking for encrypted file: {encrypted_file}")
        if os.path.exists(encrypted_file):
            print(f"  ✓ Encrypted file exists (size: {os.path.getsize(encrypted_file)} bytes)")
        else:
        ...
        print("\n=== Testing Upload ===")
        try:
            receipt = asset.upload(cfg, "cdn")
            print(f"✓ Upload method called successfully")
            ...
            if receipt:
                ...
                if receipt.success:
                    print(f"\n🎉 UPLOAD SUCCESS!")
                    print(f"   Uploaded to: https://localhost:8443/test/enc/{asset.storage_hash}.enc")
                else:
                    print(f"\n❌ Upload failed: {receipt.status_message}")
            else:
