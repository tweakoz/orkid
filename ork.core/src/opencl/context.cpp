...
std::string cl_errorstr(cl_int status){
  switch(status){
...
kernel_ptr_t Context::createKernelFromString(const std::string& name, const std::string& source) {
  ...
  cl_program program     = clCreateProgramWithSource(impl->_context, 1, 
  ...
  if(status != CL_SUCCESS){
    auto errstr = cl_errorstr(status);
    printf("OpenCL build error: %s\n", errstr.c_str());
    // get error
    size_t log_size = 0;
    status = clGetProgramBuildInfo(program, impl->_device_id, CL_PROGRAM_BUILD_LOG, 0, nullptr, &log_size);
    OrkAssert(status == CL_SUCCESS);
    std::string log;
    log.resize(log_size);
    status = clGetProgramBuildInfo(program, impl->_device_id, CL_PROGRAM_BUILD_LOG, log_size, (void*)log.data(), nullptr);
    OrkAssert(status == CL_SUCCESS);
    printf("OpenCL build error: %s\n", log.c_str());
    OrkAssert(false);
  }
