...
#if defined( WIN32 )
	if( etype == EPATHTYPE_NATIVE ) etype = EPATHTYPE_DOS;
#else
	if( etype == EPATHTYPE_NATIVE ) etype = EPATHTYPE_POSIX;
#endif
...
void Path::ComputeMarkers(char pathsep)
{
	...
	int itot = mMarkers.mDriveLen + mMarkers.mUrlBaseLen + mMarkers.mFolderLen + mMarkers.mFileNameLen + mMarkers.mExtensionLen + mMarkers.mQueryStringLen;
...
void Path::Compose(	const ork::file::Path::SmallNameType &url,
						...
						const ork::file::Path::NameType &query )
{
	...
	if( iul ) str.append( url.c_str(), iul );
	if( idl ) str.append( drive.c_str(), idl );
	if( ifl ) str.append( folder.c_str(), ifl );
	if( igl ) str.append( file.c_str(), igl );
	if( iel )
	{	str.append( ".", 1 );
...
void Path::DeCompose(	ork::file::Path::SmallNameType &url,
