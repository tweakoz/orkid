...
void FileDevContext::setFilesystemBaseAbs(file::Path base) {
  SetFilesystemBaseAbs(base.c_str());
}
void FileDevContext::setFilesystemBaseRel(file::Path base) {
  SetFilesystemBaseRel(base.c_str());
}
...
void FileDevContext::SetFilesystemBaseAbs(file::Path::NameType base) {
  ...
  file::Path::NameType nbase = baseisabs ? file::Path::NameType(base) : startupdir + file::Path::NameType(base);
  ...
  // printf( "SetFilesystemBaseAbs<%s> : abs<%s> rel<%s> baseisabs<%d>\n", base.c_str(), msFilesystemBaseAbs.c_str(),
  // msFilesystemBaseRel.c_str(), int(baseisabs) );
}
...
void FileDevContext::CreateToc(const file::Path::SmallNameType& UrlName) {
  if (meTocMode == ETM_CREATE_TOC) {
    ...
    mTocString = ork::CreateFormattedString(
        ...
        UrlBase.c_str());
    mTocString +=
        ork::CreateFormattedString("void FillToc_%s( orkmap<ork::file::Path::NameType,int>& the_map ) {\n", UrlBase.c_str());
  ...
  if (kPerformBuffering && rFile.IsBufferingEnabled()) {
    ...
    while (ilefttoread) { //////////////////////////////////////////////////////////
      ...
      if (inumqueued) {
        ...
        if (ithiscnt > ilefttoread)
          ithiscnt = ilefttoread;
