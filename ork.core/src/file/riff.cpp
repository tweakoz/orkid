...
U32 RIFFChunk::ChunkName(U8 a, U8 b, U8 c, U8 d) {
  U32 uval = ((static_cast<U32>(a) & 255)) + ((static_cast<U32>(b) & 255) << 8) + ((static_cast<U32>(c) & 255) << 16) +
...
S32 RIFFFile::ReadVarLenInt(U8* buf, U32* size) {
  *size = 0;
  ...
  do {
    ...
    (*size)++;
    if (*size > 6)
      fatalerror("readVAR()", "unterminated variable integer");
    else
      rval = S32((rval << 7) + (l & 0x7f));
  } while ((l & 0x80));
  return rval;
}
...
void RiffFile2::Open(const ork::file::Path& pth) {
  ...
  while (ItemStack.empty() == false) {
    ...
    while (ipos < item.miLength) {
      ...
      memcpy((void*)&chunktype, item.mpBase + ipos, 4);
      ipos += 4;
      memcpy((void*)&chunklength, item.mpBase + ipos, 4);
      ipos += 4;
