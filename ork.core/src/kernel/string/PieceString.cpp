...
const char* PieceString::c_str() const {
  return mpString;
}
PieceString::size_type PieceString::length() const {
  return mLength;
}
PieceString::size_type PieceString::size() const {
  return mLength;
}

PieceString::const_iterator PieceString::begin() const {
  return c_str();
}
PieceString::const_iterator PieceString::end() const {
  return c_str() + size();
}

bool PieceString::empty() const {
  return 0 == size();
}

bool PieceString::operator==(const PieceString& other) const {
...
PieceString PieceString::substr(PieceString::size_type index, PieceString::size_type amount) const {
  if (length() < index || index == npos)
    index = length();
  if (npos == amount || amount > length() - index)
    amount = length() - index;
  return PieceString(c_str() + index, amount);
}
...
PieceString::size_type PieceString::find(const char* s, PieceString::size_type pos, PieceString::size_type len) const {
  if (len == npos)
    len = ::strlen(s);
  while (pos < length()) {
    if (::strncmp(mpString + pos, s, len) == 0)
      return pos;
    pos++;
  }
  return npos;
...
PieceString::size_type PieceString::find_first_of(const ConstString& s, PieceString::size_type pos) const {
  while (pos < length()) {
    if (::strchr(s.c_str(), mpString[pos]))
      return pos;
    pos++;
  }
  return npos;
...
int PieceString::compare(const PieceString& other) const {
  ...
  int compare             = ::memcmp(c_str(), other.c_str(), common_length);
  if (compare == 0)
    compare = int(length() - other.length());
