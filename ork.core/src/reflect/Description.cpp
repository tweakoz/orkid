	...
	while(deserializer.BeginCommand(command))
	{
		if(command.Type() != Command::EPROPERTY)
		{
			orkprintf("Description::DeserializeProperties:: got command %s, wanted EPROPERTY\n",
				command.Type() == Command::EOBJECT    ? "EOBJECT"  :
				command.Type() == Command::EITEM      ? "EITEM"    :
				command.Type() == Command::EATTRIBUTE ? "EATTRIBUTE" : "???");
		}

		OrkAssertI(command.Type() == Command::EPROPERTY,
			"Description::DeserializeProperties: expected a property!");
		...
		if(command.Type() == Command::EPROPERTY)
		{
			...
			{
				OrkAssertI(prop,
				"Description::DeserializeProperties: could not skip property!");
				return false;
			}
		}
		else
