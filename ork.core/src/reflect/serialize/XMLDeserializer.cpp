...
bool XMLDeserializer::BeginCommand(Command &command)
{
    ...
    if(!mbReadingAttributes)
    {
		...
		else if(!CheckLoose(" < reference ", checksize) 
			&& !CheckLoose(" < backreference ", checksize) 
			&& !CheckLoose(" </ ", checksize) 
			&& MatchLoose(" < "))
		{
...
bool XMLDeserializer::DiscardData()
{
	...
	while(
		...
		|| Deserialize(a_castable)
		|| (!mbReadingAttributes 
		   && ((ReadWord(a_string) != 0) || EatBinaryData())))
	{
		result = true;
...
bool XMLDeserializer::DiscardCommandOrData(bool &error)
{
	...
	{
		if(command.Type() == Command::EATTRIBUTE && command.Name() == "id" 
			&& command.PreviousCommand()->Type() == Command::EOBJECT)
		{
			int assigned_id;
...
bool XMLDeserializer::Check(const PieceString &s)
{
    ...
    if(std::strncmp(s.data(), buf, s.length()) == 0)
    {
        return true;
    }
    else
...
bool XMLDeserializer::CheckLoose(const PieceString &str, size_t &matchlen)
{
	...
	while(s.length())
	{
		...
		else
		{
			...
			if(std::strncmp(buf + matchlen, word.data(), word.length()) == 0)
			{
...
bool XMLDeserializer::BeginTag(const PieceString &tagname)
{
	...
	pattern.format(" < %.*s ", tagname.length(), tagname.data());

	if(MatchLoose(pattern))
	{
		if(MatchLoose(" > ")) mbReadingAttributes = false;
		else mbReadingAttributes = true;
...
bool XMLDeserializer::EndAttribute()
{
	...
	if('\'' == mAttributeEndChar) result = Match("'");
	if('"'  == mAttributeEndChar) result = Match("\"");
	if('\0' == mAttributeEndChar) result = true;
...
bool XMLDeserializer::ReadText(StringType &text)
{
	char c;
	bool reading_attribute = mCurrentCommand && mCurrentCommand->Type() == Command::EATTRIBUTE;
