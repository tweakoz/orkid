...
{
	m_SR[0] = GRIDSIZE / extends.GetSize().x;
	m_SR[1] = GRIDSIZE / extends.GetSize().y;
	m_SR[2] = GRIDSIZE / extends.GetSize().z;
	...
	const float dx = (p2.x - p1.x) / GRIDSIZE;
	const float dx_reci = 1.0f / dx;
	const float dy = (p2.y - p1.y) / GRIDSIZE;
	const float dy_reci = 1.0f / dy;
	const float dz = (p2.z - p1.z) / GRIDSIZE;
	...
	for ( p = 0; p < inump; p++ )
	{
		...
		int x1 = (int)((bv1.x - p1.x) * dx_reci);
		int x2 = (int)((bv2.x - p1.x) * dx_reci) + 1;
		x1 = (x1 < 0)?0:x1;
		x2 = (x2 > (GRIDSIZE - 1))?GRIDSIZE - 1:x2;
		int y1 = (int)((bv1.y - p1.y) * dy_reci);
		int y2 = (int)((bv2.y - p1.y) * dy_reci) + 1;
		y1 = (y1 < 0)?0:y1;
		y2 = (y2 > (GRIDSIZE - 1))?GRIDSIZE - 1:y2;
		int z1 = (int)((bv1.z - p1.z) * dz_reci);
		int z2 = (int)((bv2.z - p1.z) * dz_reci) + 1;
		z1 = (z1 < 0)?0:z1;
		...
		for ( int x = x1; x < x2; x++ )
		{
			for ( int y = y1; y < y2; y++ )
			{
				for ( int z = z1; z < z2; z++ )
				{
					...
					fvec3 pos( p1.x + x * dx, p1.y + y * dy, p1.z + z * dz );
