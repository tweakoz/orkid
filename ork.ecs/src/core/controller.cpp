...
void Controller::installRenderCallbackOnEzApp(lev2::orkezapp_ptr_t ezapp) {
  ...
  auto copy_of_onupdate = ezapp->_app_on_update;
  ezapp->onUpdate([this,copy_of_onupdate](ui::updatedata_ptr_t updata) { //
    // on main interpreter
    copy_of_onupdate(updata);
    // on sub interpreter
    _subpycontext->bindSubInterpreter();
    this->update();
    _subpycontext->unbindSubInterpreter();
    // on main interpreter
    this->executeDeferredInvokations();
  });
}
...
void Controller::updateInit(){
  _subpycontext = std::make_shared<python::SubContext>();
}
...
void Controller::update() {
    ...
    _simulation.atomicOp([this](simulation_ptr_t& unlocked) {
      unlocked->_update();
      _deferred_script_invokations = unlocked->dequeueDeferredInvokations();
    });
...
void Controller::executeDeferredInvokations(){
    for (auto item : _deferred_script_invokations) {
      item->_cb(item->_data);
    }
}
