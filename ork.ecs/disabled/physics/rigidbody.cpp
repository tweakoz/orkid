...
void RigidBody::IntegrateImpulses(float fdt) {
  ...
  fquat qav;
  qav.fromAxisAngle(fvec4(av_axis, av_mag * fdt));
  qav.Normalize();
...
float RigidBody::SingleBodyImpulse(
    const fvec3& cn, // collision normal (world space)
...
float RigidBody::DualBodyImpulse(
    const fvec3& cn,
    const fvec3& cp,
    const RigidBody& rb1,
    const RigidBody& rb2,
  ...
  float ImpulseNumerator = -v_dot_cn * (1.0f + felas);
  ////////////////////////////////////////////////
  float ndnm = ((1.0f / rb1mass) + (1.0f / rb2mass)) * cn.Dot(cn);
  ////////////////////////////////////////////////
  ...
  float ImpulseDenominator = ndnm + (t21 + t22).Dot(cn);
  ////////////////////////////////////////////////
  float impulse = (ImpulseNumerator / ImpulseDenominator);
