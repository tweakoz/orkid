...
################################################################################
# ECS FPS2 - FPS with projectile-based ball despawning
...
GROUP_ENV = 4
GROUP_PROJECTILE = 8
GROUP_ALL = GROUP_PLAYER | GROUP_BALL | GROUP_ENV | GROUP_PROJECTILE
NUM_BALLS = 1000
OFFSET = vec3(0,0.0,0)
SIMRATE = 60
...
class ECS_FIRST_PERSON_SHOOTER(object):
  ...
  def createPlayerData(self):
    ...
    c_physics.groupCollidesWith = GROUP_BALL|GROUP_ENV|GROUP_PROJECTILE
  ...
  def createBallData(self):
    ...
    ############################
    # despawn ball on projectile hit
    ############################

    def onBallCollision(table):
      ga = table[tokens.groupA]
      gb = table[tokens.groupB]
      if ga == GROUP_PROJECTILE:
        ball_ref = table[tokens.entrefB]
        proj_ref = table[tokens.entrefA]
      elif gb == GROUP_PROJECTILE:
        ball_ref = table[tokens.entrefA]
        proj_ref = table[tokens.entrefB]
      else:
        return
      if ball_ref.id not in self._despawned:
        self._despawned.add(ball_ref.id)
        self.controller.despawnEntity(ball_ref)
        self.spawncounter -= 1
      if proj_ref.id not in self._despawned:
        self._despawned.add(proj_ref.id)
        self.controller.despawnEntity(proj_ref)

    c_physics.onCollision(onBallCollision)
  ...
  def createProjectileData(self):
    ...
    c_physics.disablePhysics = False
    c_physics.shape = sphere
    c_physics.groupAssign = GROUP_PROJECTILE
