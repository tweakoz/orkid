...
void TuioServer::sendFullMessages() {
  ...
  for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject != objectList.end(); tuioObject++) {
    ...
    (*fullPacket) << (int32)((*tuioObject)->getSessionID()) << (*tuioObject)->getSymbolID() << (*tuioObject)->getX()
                  << (*tuioObject)->getY() << (*tuioObject)->getAngle();
    (*fullPacket) << (*tuioObject)->getXSpeed() << (*tuioObject)->getYSpeed() << (*tuioObject)->getRotationSpeed()
                  << (*tuioObject)->getMotionAccel() << (*tuioObject)->getRotationAccel();
    (*fullPacket) << osc::EndMessage;
  }
  // add the immediate fseq message and send the object packet
  (*fullPacket) << osc::BeginMessage("/tuio/2Dobj") << "fseq" << -1 << osc::EndMessage;
...
TuioObject* TuioServer::addTuioObject(int f_id, float x, float y, float a) {
  ...
  if (verbose)
    std::cout << "add obj " << tobj->getSymbolID() << " (" << tobj->getSessionID() << ") " << tobj->getX() << " " << tobj->getY()
...
void TuioServer::updateTuioObject(TuioObject* tobj, float x, float y, float a) {
  ...
  if (verbose && tobj->isMoving())
    std::cout << "set obj " << tobj->getSymbolID() << " (" << tobj->getSessionID() << ") " << tobj->getX() << " " << tobj->getY()
              << " " << tobj->getAngle() << " " << tobj->getXSpeed() << " " << tobj->getYSpeed() << " " << tobj->getRotationSpeed()
              << " " << tobj->getMotionAccel() << " " << tobj->getRotationAccel() << std::endl;
}
...
TuioCursor* TuioServer::addTuioCursor(float x, float y) {
  ...
  if (verbose)
    std::cout << "add cur " << tcur->getCursorID() << " (" << tcur->getSessionID() << ") " << tcur->getX() << " " << tcur->getY()
              << std::endl;
...
void TuioServer::addObjectMessage(TuioObject* tobj) {
  ...
  (*oscPacket) << tobj->getXSpeed() << tobj->getYSpeed() << tobj->getRotationSpeed() << tobj->getMotionAccel()
...
void TuioServer::stopUntouchedMovingObjects() {
  ...
  for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject != objectList.end(); tuioObject++) {
    ...
    if ((tobj->getTuioTime() != currentFrameTime) && (tobj->isMoving())) {
      ...
      updateObject = true;
      if (verbose)
        std::cout << "set obj " << tobj->getSymbolID() << " (" << tobj->getSessionID() << ") " << tobj->getX() << " "
                  << tobj->getY() << " " << tobj->getAngle() << " " << tobj->getXSpeed() << " " << tobj->getYSpeed() << " "
                  << tobj->getRotationSpeed() << " " << tobj->getMotionAccel() << " " << tobj->getRotationAccel() << std::endl;
    }
  }
}
    ...
    if ((tcur->getTuioTime() != currentFrameTime) && (tcur->isMoving())) {
      ...
      if (verbose)
        std::cout << "set cur " << tcur->getCursorID() << " (" << tcur->getSessionID() << ") " << tcur->getX() << " "
                  << tcur->getY() << " " << tcur->getXSpeed() << " " << tcur->getYSpeed() << " " << tcur->getMotionAccel() << " "
                  << std::endl;
    }
  }
}
