...
def createParticleData( ):
  ...
  class ImplObject(object):
    def __init__(self):
      ...
      self.sprites       = self.graphdata.create("STRK",particles.SpriteRenderer)
 
      self.ptc_pool.pool_size = 5000 # max number of particles in pool
      ...
      #self.sprites.inputs.Length = .02
      #self.sprites.inputs.Width = .05
...
class ParticlesApp(object):
  ...
  def onGpuInit(self,ctx):
    ...
    #self.material.modulation_texture = Texture.load("src://effect_textures/uvmap_A");
  ...
  def onUpdate(self,updinfo):
    ...
    if hasattr(self.ptc_data,"elliptical"):
      ...
      sine = math.sin(T)
      amt = 1.0/16.0
      P1 = vec3(0,amt+amt*sine,0)
      P2 = vec3(0,-amt-amt*sine,0)
      ...
      P1 = vec3(0,0.5*sine+1.0,0)
      P2 = vec3(0,-0.5*sine-1.0,0)
      scale = (P1-P2).length+1.0

      RENDERER = self.ptc_data.sprites.inputs
      RENDERER.Size = 2*scale
      #RENDERER.Width = .025/scale

      TRB = self.ptc_data.turbulence.inputs
      TRB.Amount = vec3(1.5)


      scale = (P1-P2).length*4.0+1.0
      self.particlenode.worldTransform.scale = scale
    ##########################################
