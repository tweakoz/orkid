...
def presetTURB1(turbulence):
  turbulence.inputs.Amount = vec3(1.5,1.5,1.5)*0.1

################################################

def presetVORT1(vortex):
  vortex.inputs.VortexStrength = 0.03
  vortex.inputs.OutwardStrength = 0.03
...
def presetGRAV1(gravity):
  ...
  gravity.inputs.Mass = 0.1
  gravity.inputs.OthMass = 0.1
...
def presetEMITN1(emitter):
  ...
  emitter.inputs.EmissionRate = 20
  emitter.inputs.EmissionVelocity = 0.3
...
def createParticleData( owner, 
    ...
    owner.drawable_data.emitterIntensity = 0.0
...
def createDefaultSpriteSystem(app=None):
    ...
    app.SPRI.inputs.Size = 0.03
    app.SPRI.inputs.GradientIntensity = 1
    app.SPRI.material = presetMaterial(grad=presetGRAD(3))
