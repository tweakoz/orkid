...
struct SliceAnimationData {
  int slice_index;
  int tex_size;
  fvec2 scroll_direction;
  fvec2 scroll_speed;
  std::shared_ptr<Image> current_image;
  std::shared_ptr<Image> base_image;
...
struct Resources {
  Resources(Context* ctx) : _ctx(ctx) {
    auto txi = ctx->TXI();
    ...
    _slice_animations[0]->scroll_direction = fvec2(0, 1);  // scroll up
    _slice_animations[0]->scroll_speed = fvec2(0, 30);     // pixels per second
    ...
    _slice_animations[1]->scroll_direction = fvec2(1, 0);  // scroll right
    _slice_animations[1]->scroll_speed = fvec2(45, 0);     // pixels per second
    ...
    _slice_animations[2]->scroll_direction = fvec2(1, 1).normalized();  // scroll diagonally
    _slice_animations[2]->scroll_speed = fvec2(35, 35);
    ...
    _slice_animations[3]->scroll_direction = fvec2(-1, -0.5f).normalized();  // scroll left and down
    _slice_animations[3]->scroll_speed = fvec2(50, 25);
  ...
  void startAnimationThreads() {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      _animation_threads[i]->start([this, i](anyp data) {
        animateSlice(i);
      });
    }
  }
  ...
  void animateSlice(int slice_idx) {
    ...
    while (_running) {
      ...
      // Wrap scroll values
      while (anim_data->accumulated_scroll_x >= anim_data->tex_size) {
        anim_data->accumulated_scroll_x -= anim_data->tex_size;
      }
      while (anim_data->accumulated_scroll_x < 0) {
        anim_data->accumulated_scroll_x += anim_data->tex_size;
      }
      while (anim_data->accumulated_scroll_y >= anim_data->tex_size) {
        anim_data->accumulated_scroll_y -= anim_data->tex_size;
      }
      while (anim_data->accumulated_scroll_y < 0) {
        anim_data->accumulated_scroll_y += anim_data->tex_size;
      }
      ...
      // Update image data with scrolling
      {
        ...
        anim_data->needs_update = true;
      }
      
      // Sleep to avoid consuming too much CPU
      usleep(16667); // ~60 FPS update rate
    }
  }
      ...
      case ui::EventCode::KEY_DOWN:
        switch (ev->miKeyCode) {
          case 'R':
            ...
            for (auto& anim : resources->_slice_animations) {
              anim->accumulated_scroll_x = 0.0f;
              anim->accumulated_scroll_y = 0.0f;
            }
            break;
          case '+':
          case '=':
