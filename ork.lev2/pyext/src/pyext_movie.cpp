...
void pyinit_movie(py::module& module_lev2) {
  ...
  auto movieplayback_type = //
      py::class_<MoviePlaybackContext, movieplayback_ptr_t>(module_lev2, "MoviePlaybackContext")
          ...
          .def_property_readonly("current_frame_index", [](movieplayback_ptr_t ctx) -> int64_t { return ctx->_current_frame_index; })
          ...
          // Audio metadata
          .def_property_readonly("audio_sample_rate", [](movieplayback_ptr_t ctx) -> int {
            if (ctx->_audio_config && ctx->_audio_config->_valid) {
              return ctx->_audio_config->_sample_rate;
            }
            return 0;
          })
          .def_property_readonly("audio_channels", [](movieplayback_ptr_t ctx) -> int {
            if (ctx->_audio_config && ctx->_audio_config->_valid) {
              return ctx->_audio_config->_num_channels;
            }
            return 0;
          })
          .def_property_readonly("audio_codec_name", [](movieplayback_ptr_t ctx) -> std::string {
            if (ctx->_audio_config && ctx->_audio_config->_valid) {
              return ctx->_audio_config->_codec_name;
            }
            return "";
          })
          .def_property_readonly("has_audio", [](movieplayback_ptr_t ctx) -> bool {
            return ctx->_audio_config && ctx->_audio_config->_valid;
          })
          // Format metadata
          .def_property_readonly("duration", [](movieplayback_ptr_t ctx) -> double { return ctx->_duration; })
          .def_property_readonly("bit_rate", [](movieplayback_ptr_t ctx) -> int64_t {
            if (ctx->_format_ctx) {
              return ctx->_format_ctx->bit_rate;
            }
            return 0;
          })
          .def_property_readonly("nb_streams", [](movieplayback_ptr_t ctx) -> unsigned int {
            if (ctx->_format_ctx) {
              return ctx->_format_ctx->nb_streams;
            }
            return 0;
          })
          .def_property_readonly("format_name", [](movieplayback_ptr_t ctx) -> std::string {
            if (ctx->_format_ctx && ctx->_format_ctx->iformat && ctx->_format_ctx->iformat->name) {
              return ctx->_format_ctx->iformat->name;
            }
            return "";
          })
          .def_property_readonly("format_long_name", [](movieplayback_ptr_t ctx) -> std::string {
          ...
          .def_property_readonly("video_codec_name", [](movieplayback_ptr_t ctx) -> std::string {
