...
void pyinit_gfx_xgmmodel(py::module& module_lev2) {
  ...
  auto model_type_t = py::class_<XgmModel, xgmmodel_ptr_t>(module_lev2, "XgmModel") //
                          .def(py::init([]() -> xgmmodel_ptr_t {
                            auto modl = std::make_shared<XgmModel>();
                            return modl;
                          }))
                          ...
                          .def(
                              "createMesh",                                                 //
                              [](xgmmodel_ptr_t model, std::string name) -> xgmmesh_ptr_t { //
                                auto mesh = std::make_shared<XgmMesh>();
                                auto ps   = AddPooledString(name.c_str());
                                model->AddMesh(ps, mesh);
                                return mesh;
                              })
                          .def(
                              "createInstancedNode",   //
                          ...
                          .def_property_readonly("aabb_xyz", [](xgmmodel_ptr_t model) -> fvec3 { return model->mAABoundXYZ; })
                          .def_property_readonly("aabb_whd", [](xgmmodel_ptr_t model) -> fvec3 { return model->mAABoundWHD; })
                          .def("get_bounding_box", [](xgmmodel_ptr_t model) -> AABox { return model->GetBoundingBox(); });
  ...
  auto mesh_type_t = py::class_<XgmMesh, xgmmesh_ptr_t>(module_lev2, "XgmMesh")
                         ...
                         .def(
                             "createSubMeshWithMaterial",                                     //
                             [](xgmmesh_ptr_t mesh, material_ptr_t mtl) -> xgmsubmesh_ptr_t { //
                               auto submesh = std::make_shared<XgmSubMesh>();
                               mesh->AddSubMesh(submesh);
                               submesh->_material = mtl;
                               return submesh;
                             })
                         .def(
                             "createSubMeshWithPipeline",                                        //
                             [](xgmmesh_ptr_t mesh, fxpipeline_ptr_t pipe) -> xgmsubmesh_ptr_t { //
                               auto submesh = std::make_shared<XgmSubMesh>();
                               mesh->AddSubMesh(submesh);
                               submesh->_pipelineOverride = pipe;
                               return submesh;
                             });
  ...
  auto submesh_type_t = py::class_<XgmSubMesh, xgmsubmesh_ptr_t>(module_lev2, "XgmSubMesh")
                            .def(
                                "createCluster",                                                               //
                                [](xgmsubmesh_ptr_t submesh, vtxbufferbase_ptr_t vtxbuf) -> xgmcluster_ptr_t { //
                                  auto cluster           = std::make_shared<XgmCluster>();
                                  cluster->_vertexBuffer = vtxbuf; // owning
                                  cluster->meVtxStrFmt   = vtxbuf->meStreamFormat;
                                  cluster->mBoundingBox  = vtxbuf->_aabb;
                                  submesh->_clusters.push_back(cluster);
                                  return cluster;
                                })
                            .def_property_readonly(
                                "clusters",                                //
  ...
  auto xgmclus_t =
      py::class_<XgmCluster, xgmcluster_ptr_t>(module_lev2, "XgmCluster")
          .def(
              "createPrimGroup",                                                                                         //
              [](xgmcluster_ptr_t cluster, crcstring_ptr_t primtype, idxbufferbase_ptr_t idxbuf) -> xgmprimgroup_ptr_t { //
                auto primgroup            = std::make_shared<XgmPrimGroup>();
  ...
  type_codec->registerStdCodec<xgmcluster_ptr_t>(xgmclus_t);
  /////////////////////////////////////////////////////////////////////////////////
  auto pg_type = py::class_<XgmPrimGroup, xgmprimgroup_ptr_t>(module_lev2, "XgmPrimGroup")
                     .def_property(
                         "primitive",
                         [](xgmprimgroup_ptr_t primgroup) -> crcstring_ptr_t { //
                           auto crcs     = std::make_shared<CrcString>();
                           crcs->_hashed = uint64_t(primgroup->mePrimType);
                           return crcs;
                         },
                         [](xgmprimgroup_ptr_t primgroup, crcstring_ptr_t prim) { //
                           primgroup->mePrimType = PrimitiveType(prim->hashed()); //
                         })
                     .def_property_readonly(
                         "indexBuffer",                                        //
                         [](xgmprimgroup_ptr_t primgroup) -> idxbuf_rawptr_t { //
                           return idxbuf_rawptr_t(primgroup->mpIndices);
                         });
  type_codec->registerStdCodec<xgmprimgroup_ptr_t>(pg_type);
  /////////////////////////////////////////////////////////////////////////////////
  auto submeshinst_type_t =
      py::class_<XgmSubMeshInst, xgmsubmeshinst_ptr_t>(module_lev2, "XgmSubMeshInst")
          .def_property_readonly("material", [](xgmsubmeshinst_ptr_t smi) -> material_ptr_t { return smi->material(); })
          .def(
              "overrideMaterial",                                      //
  ...
  /////////////////////////////////////////////////////////////////////////////////
  auto modelinst_type_t =
      py::class_<XgmModelInst, xgmmodelinst_ptr_t>(module_lev2, "XgmModelInst")
          .def(py::init([](xgmmodel_ptr_t model) -> xgmmodelinst_ptr_t { return std::make_shared<XgmModelInst>(model.get()); }))
          .def(
              "enableSkinning",              //
