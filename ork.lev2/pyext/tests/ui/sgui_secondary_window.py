...
################################################################################
# DualSceneComponent - all scene/window logic as a component
################################################################################

class _DualSceneComponent(ApplicationComponent):
  ...
  def _setup_secondary_window(self, ctx):
    ...
    win_width = self.app.win_width
    win_height = self.app.win_height

    sec_x = 100 + win_width + 20
    self.secondary_win = ezapp.createSecondaryWindow(
      width=win_width,
      height=win_height,
      x=sec_x,
      y=100,
  ...
  def _onGpuInit(self, ctx):
    print("GPU init")
    self._setup_theme_engine(self.app.ezapp.uicontext)
...
class DualSceneGraphWindow(ComponentizedApplication):
