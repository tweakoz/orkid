...
################################################################################
# D2 gate: DflowEditor default dock layout — DockSpace adoption reproduces the
...
#  are each built at the dflow editor's EXACT default params (viewport fill; left
#  column split LEFT @0.4; property sheet split BOTTOM @0.55; per-panel split
#  margin 2; matching titlebar colors + titles). Panel BODIES are solid Boxes
#  (deterministic — isolates the layout, not the live viewport content).
#
...
from ork.editor.dflowedit import (
...
TB_V  = vec4(0.15, 0.20, 0.25, 1)
TB_L  = vec4(0.20, 0.15, 0.20, 1)
TB_P  = vec4(0.20, 0.20, 0.15, 1)
...
def compare(a, b):
  ...
  print(f"COMPARE differing={differing} maxdiff={int(diff.max())}", flush=True)
  return (differing == 0), differing

def orchestrate():
  sp = os.path.abspath(__file__)
  base_png = "/tmp/dflowedit_parity_baseline.png"
  dock_png = "/tmp/dflowedit_parity_dockspace.png"
  ...
  if problems:
    print("=== DflowEditor layout-parity gate FAILED ===", flush=True)
    for p in problems: print("  - " + p, flush=True)
    sys.exit(1)
  print("=== DflowEditor layout-parity gate PASSED (byte-equal) ===", flush=True)
  sys.exit(0)
...
def main():
  ...
  args = ap.parse_args()
  if args.mode:
    run_capture(args.mode, args.capture or f"/tmp/dflowedit_parity_{args.mode}.png")
  orchestrate()
