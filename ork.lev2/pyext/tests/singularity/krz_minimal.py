...
# Parse arguments
################################################################################
parser = argparse.ArgumentParser(description='KRZ Minimal Test')
...
if args.newui:
    ...
    class KrzApp(SingulTestAppNewUI):
        ...
        def _load_krz_data(self):
            """Load KRZ soundbank and set up programs."""
            ...
            for item in layers:
                createSampleLayer(
                    newprog,
                    multisample=self.krzsamps[item["name"]],
                    lokey=item["lokey"],
                    hikey=item["hikey"],
                    lowpass=item["lowpass"],
                    tuning=item["tuning"])
