...
def main():
  ...
  hf = HFAsset(dsl_file="strata_bake", dsl_class="StrataBake", dimension=512,
               extent_m=4096.0, height_scale_m=1000.0, ctx=ctx)
  ...
  for ch in ("stripes", "mottle"):
    present, st = show(ch)
    ok = ok and present and st is not None and (st.max - st.min) > 1e-3
  ...
  ok = ok and cp and np_ and comp_narrow and norm_full
