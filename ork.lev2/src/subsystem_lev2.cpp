...
subsystem_ptr_t createLev2Subsystem() {
  ...
  // INITIALIZING state -> transitions to READY immediately
  // (dependencies are already ready when this runs)
  subsystem->_state_initializing->_onenter = [](fsm::fsminstance_ptr_t instance) {
    logchan_LEV2->log("Lev2 meta-service: all lev2 subsystems ready");
    instance->sendEvent("READY");
  };

  // SHUTTING_DOWN state -> transitions to TERMINATED
  subsystem->_state_shutting_down->_onenter = [](fsm::fsminstance_ptr_t instance) {
