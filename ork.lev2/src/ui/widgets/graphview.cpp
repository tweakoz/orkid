...
/////////////////////////////////////////////////////////////////////////
GraphView::GraphView()
    : Surface("GraphView", 0, 0, 32, 32, fvec3(1, 0, 0), 1.0), _lockX(false),
      _lockY(false), _dragging(false), _lockYZOOM(false), _fontsize(16) {
...
HandlerResult GraphView::DoOnUiEvent(event_constptr_t ev) {
  ...
  case ui::EventCode::DOUBLECLICK: {
    ...
    if (numchannels) {
      int maxy = numchannels * _fontsize + _kbasechanlaby + _fontsize;
      if (ilocx > (width() - (_fontsize * 4)) and ilocy < maxy) {
        int ichannel = (ilocy - _fontsize) >> 4;
...
static vtxbuf_ptr_t create_vertexbuffer(lev2::Context *context) {
  auto vb = std::make_shared<vtxbuf_t>(16 << 20, 0,
                                       lev2::PrimitiveType::NONE); // ~800 MB
  vb->SetRingLock(true);
  return vb;
}
...
void GraphView::DoRePaintSurface(drawevent_constptr_t drwev) {
  ...
  mtxi->PushUIMatrix();
  {
    ...
    if (0)
      printf("drawbox<%s> xy1<%d,%d> xy2<%d,%d>\n", //
             _name.c_str(), ix1, iy1, ix2, iy2);
    ...
    if (0) { // alphabg
      ...
      primi.RenderQuadAtZ(defmtl.get(), tgt,
                          ix1,  // x0
                          ix2,  // x1
                          iy1,  // y0
                          iy2,  // y1
    ...
    for (auto channel : _channelmap) {
      ...
      lev2::FontMan::DrawText(tgt, //
                              ix2 - (sw + _fontsize), ichanlaby,
                              channel->_name.c_str());
      ...
      if (channel->_visible) {
        ...
        if (numpoints) {
          ...
          lev2::FontMan::beginTextBlock(tgt, 128);
          lev2::FontMan::DrawText(tgt, //
                                  ix2 - (sw + _fontsize) - (sw2 + _fontsize),
                                  ichanlaby, valstr.c_str());
          lev2::FontMan::endTextBlock(tgt);
        }
        ...
        int x1 = ix2 - (sw + _fontsize);
        int x2 = x1 + sw;
        int y1 = ichanlaby;
        int y2 = ichanlaby + _fontsize;
    ...
    int iy = _fontsize;
    lev2::FontMan::DrawText(tgt, //
                            _fontsize, iy += _fontsize, "pan: left-drag");
    lev2::FontMan::DrawText(tgt, //
                            _fontsize, iy += _fontsize, "zoom: mouse-wheel");
    lev2::FontMan::endTextBlock(tgt);
    ///////////////////////////////
    tgt->RefModColor() = fvec3(0, 1, 0);
    lev2::FontMan::beginTextBlock(tgt, 128);
    lev2::FontMan::DrawText(tgt, //
                            _fontsize, iy += _fontsize, "center<%g %g>",
                            _grid._center.x, _grid._center.y);
    lev2::FontMan::DrawText(tgt, //
                            _fontsize, iy += _fontsize, "zoomfactorX<%g>",
                            _grid._zoomX);
    lev2::FontMan::DrawText(tgt, //
                            _fontsize, iy += _fontsize, "zoomfactorY<%g>",
                            _grid._zoomY);
    lev2::FontMan::endTextBlock(tgt);
    ///////////////////////////////
    if (_name.length()) {
      ...
      lev2::FontMan::DrawText(tgt, //
                              ixc - (sw >> 1), _fontsize, _name.c_str());
      lev2::FontMan::endTextBlock(tgt);
    }
    ///////////////////////////////
