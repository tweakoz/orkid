...
void ShaderNode::_generate2Common(shaderbuilder::BackEnd& backend) const {
  ...
  // visit interfaces
  //////////////////////////////////////////////
  unique_deco_set uset_interfaces;
  deco_children->interfaceNodes(uset_interfaces);
  for (auto ifnode : uset_interfaces._ordered) {
    ...
    //////////////////////////////////////////////////////////////////
      ...
      assert(it_stif != c->_storageInterfaces.end());
...
///////////////////////////////////////////////////////////
template <typename T> struct unique_collection {
  using raw_ptr_t = const T*;
  using vec_t = std::vector<raw_ptr_t>;
  using set_t = std::unordered_set<raw_ptr_t>;

  unique_collection(shaderbuilder::BackEnd& backend)
      : _backend(backend) {
  }

  /////////////////////////////////////////////////////

  void _visit_impl(decoblocknode_rawconstptr_t node) {

    _already_visited.insert(node);

    for (auto dep : node->_dependencies){
      auto it = _already_visited.find(dep.get());
      if (it != _already_visited.end())
        continue;
      _visit_impl(dep.get());
    }

    if (auto as_t = dynamic_cast<raw_ptr_t>(node)) {
      if (_uniques.find(as_t) == _uniques.end()){
  ...
  std::unordered_set<decoblocknode_rawconstptr_t> _already_visited;
};
///////////////////////////////////////////////////////////
void ShaderNode::emitShaderDeps(shaderbuilder::BackEnd& backend) const {
  ...
  unique_collection<LibraryBlockNode> libblocks(backend);
  unique_collection<UniformBlockNode> uniblocks(backend);
  unique_collection<UniformSetNode> unisets(backend);
  unique_collection<InterfaceNode> interfaces(backend);
