...
{
	if( imeshindex>= mXgmModel->GetNumMeshes() ) return false;
...
{
	if(mXgmModel->GetNumMeshes()  == 1) return true;
	
	for(int i = 0; i < knummaskbytes; i++)
		if(mMaskBits[i]) return(true);
		...
		struct RenderClus
		{
			static void RenderPrim( ork::lev2::GfxTarget* pTARG, const XgmCluster& XgmClus )
			{
				...
				for( int iprim=0; iprim<inumprim; iprim++ )
				{
					...
					pTARG->GBI()->DrawIndexedPrimitiveEML(	*pVertexBuffer,
															*pIndexBuffer,
															PrimGroup.GetPrimType()
														);
				}
#endif
										...
										const RenderContextInstModelData & mdlctx ) const
{
	...
	pTARG->PushModColor( ModColor );
	{
		...
		if( 0 != pmat )
		{
			...
			int ipass = Ctx.GetMaterialPassIndex();
			OrkAssert(ipass<inumpasses);
			bool bDRAW = pmat->BeginPass( pTARG,ipass );
...
void XgmModel::InitDisplayLists( ork::lev2::GfxTarget *pTARG )
{
#if USE_XGM_DISPLAYLISTS
	int inummeshes = GetNumMeshes();
	for( int i=0; i<inummeshes; i++ )
	{	GetMesh(i)->InitDisplayLists( pTARG );
	}
#endif

	int inummats = this->GetNumMaterials();

	for( int im=0; im<inummats; im++ )
	{
		GetMaterial(im)->Init( pTARG );
	}
...
void XgmMesh::InitDisplayLists( ork::lev2::GfxTarget *pTARG )
{
#if USE_XGM_DISPLAYLISTS
	int inumcs = GetNumSubMeshes();
	for( int i=0; i<inumcs; i++ )
	{	GetSubMesh(i)->InitDisplayLists( pTARG );
	}
#endif
}
...
void XgmSubMesh::InitDisplayLists( ork::lev2::GfxTarget *pTARG )
{
#if USE_XGM_DISPLAYLISTS
	int inumc = GetNumClusters();
	for( int i=0; i<inumc; i++ )
	{	RefCluster(i).InitDisplayList( pTARG );
	}
#endif
}
...
void XgmCluster::InitDisplayList( ork::lev2::GfxTarget *pTARG )
{
#if USE_XGM_DISPLAYLISTS
	pTARG->GBI()->DisplayListBegin( mDisplayList );
	const VertexBufferBase* vb = GetVertexBuffer();
	int inumpg = GetNumPrimGroups();
	for( int i=0; i<inumpg; i++ )
	{	
		const XgmPrimGroup&  pg = RefPrimGroup(i);
		const IndexBufferBase* ib = pg.GetIndexBuffer();
		pTARG->GBI()->DisplayListAddIndexedPrimitiveEML( mDisplayList, *vb, *ib, pg.GetPrimType() );
	}
	pTARG->GBI()->DisplayListEnd( mDisplayList );
#endif
}
