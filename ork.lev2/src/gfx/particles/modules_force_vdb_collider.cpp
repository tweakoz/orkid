...
#include <ork/math/cvector3.hpp> // fvec3 custom-serdes (sdf_offset reflection)
...
struct VdbColliderModuleInst : public ParticleModuleInst {
  ...
  void compute(GraphInst* inst, ui::updatedata_ptr_t updata) final {
    ...
    for (int i = 0; i < n; i++) {
      ...
      // SDF placement offset (local frame): the surface lives AT +offset, so the
      // particle is tested against the grid at (p - offset).
      p = p - _vrd->_sdf_offset;
...
void VdbColliderModuleData::describeX(class_t* clazz) {
  ...
  clazz->directProperty("sdf_offset", &VdbColliderModuleData::_sdf_offset);
  // the SDF asset-REFERENCE (resolved to the live grid post-deserialize); _sdfGrid
  // itself is runtime-only derived data and deliberately NOT reflected.
  clazz->directProperty("sdf_asset", &VdbColliderModuleData::_sdf_asset_name);
}
