...
void VkTextureInterface::initTextureArray2DFromData(TextureArray* array, TextureArrayInitData tid) {
  ...
  VKICI->flags = 0; // VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT if needed
  ...
  if (DEBUG_TEXARRAY2D) {
    logchan_txia2d->log("TextureArray created: maxw<%d> maxh<%d> depth<%d> format<%s>", 
                        max_w, max_h, num_slices, EBufferFormatToName(format).c_str());
    logchan_txia2d->log("///////////////////////////////////////////////////////////");
  }
}
...
void VkTextureInterface::initTextureArray2D(TextureArray* texture_array) {
  ...
  viewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D_ARRAY;
  viewInfo.format = VkFormatConverter::convertBufferFormat(format);
  viewInfo.subresourceRange.aspectMask = (format == EBufferFormat::Z32F) 
  ...
  logchan_txia2d->log("VkTextureInterface::initTextureArray2D created blank array w<%d> h<%d> slices<%d> format<%s>",
                      w, h, num_slices, EBufferFormatToName(format).c_str());
}
...
void VkTextureInterface::updateTextureArraySlice(TextureArraySliceRef* slice_ref, image_ptr_t img) {
  ...
  for (int level = 0; level < num_levels; level++) {
    auto& mip = subimg_cmipc->_levels[level];
    memcpy(staging_data + offset, mip._data->data(), mip._data->length());
    ...
    region.imageSubresource.layerCount = 1;
    region.imageExtent = {uint32_t(mip._width), uint32_t(mip._height), 1};
