...
AABuffer::~AABuffer() {
  if (mTriangleClBuffer)
    delete mTriangleClBuffer;
  if (mFragInpClBuffer)
    delete mFragInpClBuffer;
  if (mFragOutClBuffer)
    delete mFragOutClBuffer;
}
...
void RenderData::Update() {
  ...
  ork::fray3 Ray[4];
  for (int i = 0; i < 4; i++) {
    Ray[i] = ork::fray3(mFrustum.mNearCorners[i], (mFrustum.mFarCorners[i] - mFrustum.mNearCorners[i]).normalized());
  }
  ...
  for (int iy = 0; iy < miNumTilesH; iy++) {
    ...
    topn.normalizeInPlace();
    botn.normalizeInPlace();
    ...
    for (int ix = 0; ix < miNumTilesW; ix++) {
      ...
      ork::fray3 RayTL;
      RayTL.BiLerp(Ray[0], Ray[1], Ray[3], Ray[2], filx, fity);
      ork::fray3 RayTR;
      RayTR.BiLerp(Ray[0], Ray[1], Ray[3], Ray[2], firx, fity);
      ork::fray3 RayBL;
      RayBL.BiLerp(Ray[0], Ray[1], Ray[3], Ray[2], filx, fiby);
      ork::fray3 RayBR;
      RayBR.BiLerp(Ray[0], Ray[1], Ray[3], Ray[2], firx, fiby);
      ...
      ork::fvec3 N0;
      N0.lerp(LN[0], RN[0], filx);
      ork::fvec3 N1;
      N1.lerp(LN[0], RN[0], firx);
      ork::fvec3 N2;
      N2.lerp(LN[1], RN[1], firx);
      ork::fvec3 N3;
      N3.lerp(LN[1], RN[1], filx);
      ork::fvec3 F0;
      F0.lerp(LF[0], RF[0], filx);
      ork::fvec3 F1;
      F1.lerp(LF[0], RF[0], firx);
      ork::fvec3 F2;
      F2.lerp(LF[1], RF[1], firx);
      ork::fvec3 F3;
      F3.lerp(LF[1], RF[1], filx);
      ...
      lftn.normalizeInPlace();
      rhtn.normalizeInPlace();
...
static rend_srcmesh* LoadRgm(const char* pfname, RenderData& rdata) {
  ...
  poutmesh->mEye    = poutmesh->mTarget - ork::fvec3(0.0f, 0.0f, dist.magnitude());
