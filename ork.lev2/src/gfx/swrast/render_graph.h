...
struct rend_fragment {
  ...
  float mZ                         = 0.0f;
  const rend_triangle* mpPrimitive = nullptr;
  const rend_fragment* mpNext      = nullptr;
  const rend_shader* mpShader      = nullptr;
...
struct FragmentPoolNode {
  ...
  int GetNumAvailable() const {
    return (kNumFragments - mFragmentIndex);
  }
  FragmentPoolNode();
};
///////////////////////////////////////////////////////////////////////////////
...
struct FragmentCompositorREYES : public rend_fraglist_visitor {
  ...
  int miThreadID                        = 0;
  int miNumFragments                    = 0;
  float opaqueZ                         = 1.0e30f;
  const rend_fragment* mpOpaqueFragment = nullptr;
...
public:
  ...
  char* GetBufferRegion() const {
    return GetBufferRegionPriv();
  }
  NoCLFromHostBuffer();
  ~NoCLFromHostBuffer();
  int GetSize() const {
...
protected:
  char* GetBufferRegionPriv() const {
    return (miWriteIndex & 1) ? mpBuffer1 : mpBuffer0;
  }
  int miSize;
  char* mpBuffer0;
  char* mpBuffer1;
...
public:
  ...
  const char* GetBufferRegion() const {
    return GetBufferRegionPriv();
  }
  NoCLToHostBuffer();
  ~NoCLToHostBuffer();
...
protected:
  char* GetBufferRegionPriv() const {
    return (miReadIndex & 1) ? mpBuffer1 : mpBuffer0;
  }
  // cl_mem	mCLhandle;
  int miSize;
  U32 mBufferflags;
...
public:
  ...
  rend_srcmesh* mpSrcMesh = nullptr;
...
struct ClipVert {
  ...
  const ork::fvec3& Pos() const {
    return pos;
  }
  ClipVert(const ork::fvec3& tp)
      : pos(tp) {
  }
  ClipVert()
      : pos() {
...
public:
  ...
  void AddVertex(const ClipVert& v) {
    mverts[minumverts++] = v;
  }
  int GetNumVertices() const {
    return minumverts;
  }
  const ClipVert& GetVertex(int idx) const {
    return mverts[idx];
  }
  ClipPoly()
      : minumverts(0) {
...
public:
  ...
  // const orkvector<rend_triangle*>& GetPostTransformTriangles(int ibucket) const { return
  // mBuckets[ibucket].mPostTransformTriangles; } int GetNumTriangles(int ibucket) const { return
  // mBuckets[ibucket].mTriangleIndex.Fetch(); }
  const orkmap<const rend_shader*, TriangleMetaBucket*>& GetMetaBuckets() const {
    return mMetaBuckets;
  }
  orkmap<const rend_shader*, TriangleMetaBucket*>& GetMetaBuckets() {
    return mMetaBuckets;
  }
...
private:
  ...
  ////////////////////////////////////////////////////////////
  void* mSourceHash  = nullptr;
  int miNumWorkUnits = 0;
  const RenderData& mRenderData;
