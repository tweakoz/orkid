...
vertex_ptr_t SubMeshClipper::remappedVertex(vertex_ptr_t input_vtx, bool front){
  vertex_ptr_t rval = input_vtx;
  if(do_front){
    auto it = _vertex_remap.find(input_vtx->_poolindex);
    if( it != _vertex_remap.end() ){
      rval = _outsubmesh_front.vertex(it->second);
    }
    auto try_clipped = _outsubmesh_front.tryVarAs<vertex_ptr_t>(rval, "clipped_vertex");
    if( try_clipped ){
...
void SubMeshClipper::process() {
  ...
  if( true ) { // vardump
    ...
    _outsubmesh_front.visitAllEdges([&](halfedge_ptr_t e){
        dumpEdgeVars(true, e);
    });
...
void SubMeshClipper::procEdges(merged_poly_const_ptr_t input_poly, bool do_front) { //
  ...
  for (int iva = 0; iva < inuminverts; iva++) {
    ...
    logchan_clip->log( "   iva<%d> edge<%d->%d> front<%d,%d>", iva, out_vtx_a->_poolindex, out_vtx_b->_poolindex, int(is_vertex_a_front), int(is_vertex_b_front) );  
    ...
    if (is_vertex_a_front and is_vertex_b_front) {
      ...
      plstat._vertexA                                          = out_vtx_a;
      plstat._vertexB                                          = out_vtx_b;
      if(do_log)logchan_clip->log("    emit front he_ab<%p>", (void*)he_ab.get());
    } else if ((not is_vertex_a_front) and (not is_vertex_b_front)) {
      plstat._status                                        = EPlanarStatus::BACK;
      plstat._vertexA                                          = out_vtx_a;
      plstat._vertexB                                          = out_vtx_b;
      if(do_log)logchan_clip->log("    emit back he_ab<%p>", (void*)he_ab.get());
