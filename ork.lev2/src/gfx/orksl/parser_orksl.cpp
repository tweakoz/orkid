...
FnParseContext::FnParseContext(OrkSlParser* parser, const ScannerView* v)
    : _parser(parser)
    , _view(v) {
}
...
int ParsedFunctionNode::parse(OrkSlParser* parser, const ork::ScannerView& view) {
  int i         = 0;
...
void ParsedFunctionNode::emit(shaderbuilder::BackEnd& backend) const {
