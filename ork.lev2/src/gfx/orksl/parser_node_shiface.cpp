...
int InterfaceLayoutNode::parse(OrkSlParser* parser, const ScannerView& view) {
  int i               = view._start;
...
void InterfaceNode::parseIos(OrkSlParser* parser, 
...
void InterfaceNode::parse(OrkSlParser* parser, const ScannerView& view) {
...
void InterfaceNode::_generate2(shaderbuilder::BackEnd& backend) const {
  ...
  for (auto input : _inputs->_nodes) {
    ...
    pattr->_location                    = int(_sif->_inputAttributes.size());
  ...
  if (is_vtx or is_geo or is_frg) {
    ...
    for (const auto& it : _sif->_inputAttributes) {
      ...
      // pattr->_location = itloc->second;
    }

    int isort = 0;
    for (const auto& it : attr_sort_map) {
      auto attr       = it.second;
      attr->_location = isort++;
    }
  }
  ////////////////////////
}
...
void VertexInterfaceNode::_generate1(shaderbuilder::BackEnd& backend) const {
  _sif->mName          = _name;
  _sif->_interfaceType = _interfaceType;
  auto c = backend._container;
