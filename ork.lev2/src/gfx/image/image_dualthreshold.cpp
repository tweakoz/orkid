...
////////////////////////////////////////////////////////////////
// Dual threshold with bitmask control
// channel_mask bits: 0=R, 1=G, 2=B, 3=A
// if value < low_threshold: value = set_low
// if value > high_threshold: value = set_high
// else: value unchanged
////////////////////////////////////////////////////////////////

void Image::dualThreshold(float low_threshold, float set_low, float high_threshold, float set_high, uint8_t channel_mask) {
  ...
  for (size_t chunk = 0; chunk < num_chunks; chunk++) {
    auto op = [chunk, this, channel_mask, low_threshold, set_low, high_threshold, set_high, &chunkcounter]() {
          ...
          switch (this->_format) {
            ...
            case R8: {
              if (channel_mask & 0x01) { // R channel
                ...
                if (val < low_threshold) {
                  pixel[0] = uint8_t(set_low * 255.0f);
                } else if (val > high_threshold) {
                  pixel[0] = uint8_t(set_high * 255.0f);
                }
              }
              break;
            }
            ...
            case BGR8: {
              ...
              for (size_t c = 0; c < 3; c++) {
                if (channel_mask & (1 << c)) {
                  ...
                  if (val < low_threshold) {
                    pixel[c] = uint8_t(set_low * 255.0f);
                  } else if (val > high_threshold) {
                    pixel[c] = uint8_t(set_high * 255.0f);
                  }
                }
              }
              break;
            ...
            case R16UI: {
              if (channel_mask & 0x01) { // R channel
                ...
                if (val < low_threshold) {
                  pixel[0] = uint16_t(set_low * 65535.0f);
                } else if (val > high_threshold) {
                  pixel[0] = uint16_t(set_high * 65535.0f);
                }
              }
              break;
            }
            case RGB16: {
              ...
              for (size_t c = 0; c < 3; c++) {
                if (channel_mask & (1 << c)) {
                  ...
                  if (val < low_threshold) {
                    pixel[c] = uint16_t(set_low * 65535.0f);
                  } else if (val > high_threshold) {
                    pixel[c] = uint16_t(set_high * 65535.0f);
                  }
                }
              }
              break;
            ...
            case R32F: {
              if (channel_mask & 0x01) { // R channel
                ...
                if (val < low_threshold) {
                  data[pixel_index] = set_low;
                } else if (val > high_threshold) {
                  data[pixel_index] = set_high;
                }
              }
              break;
            }
            case RGB32F: {
              ...
              for (size_t c = 0; c < 3; c++) {
                if (channel_mask & (1 << c)) {
                  ...
                  if (val < low_threshold) {
                    data[pixel_index + c] = set_low;
                  } else if (val > high_threshold) {
                    data[pixel_index + c] = set_high;
                  }
                }
              }
              break;
