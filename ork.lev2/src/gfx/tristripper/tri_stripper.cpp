...
tri_stripper::triangle_strip
tri_stripper::ExtendTriToStrip(const size_t StartTriPos, const triangle_strip::start_order StartOrder) {
  typedef common_structures::const_out_arc_iterator const_tri_link_iter;
  typedef triangles_graph::node_iterator tri_node_iter;
  typedef triangles_graph::arc_t arc_t;
  typedef triangles_graph::node_t node_t;
  ...
  for (tri_node_iter TriNodeIt = (m_Triangles.begin() + StartTriPos);
    ...
    for (LinkIt = TriNodeIt->out_begin(); LinkIt != TriNodeIt->out_end(); ++LinkIt) {
      ...
      if ((Tri.StripID() != m_StripID) && (!(the_arc.terminal()->marked()))) {
...
inline tri_stripper::triangle_edge
tri_stripper::GetLatestEdge(const triangle& Triangle, const triangle_strip::start_order Order) const {
  switch (Order) {
...
void tri_stripper::MarkTriAsTaken(const size_t i) {
  typedef triangles_graph::node_iterator tri_node_iter;
  typedef common_structures::out_arc_iterator tri_link_iter;
  typedef triangles_graph::arc_t arc_t;
  typedef triangles_graph::node_t node_t;
