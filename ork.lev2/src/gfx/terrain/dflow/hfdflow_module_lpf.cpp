...
//
// RUNTIME PARAMS: sigma and blend live in a small params SSBO (si_p) read at compute,
// NOT baked into the shader text (mirrors erox/pha). Only the loop RADIUS must bake
// (GLSL loop bounds are compile-time), so it's BUCKETED to the next power of two:
...
static std::string _lpf_text(int dim, const char* name, bool horizontal, int rmax) {
  ...
  const char* idx = horizontal ? "uint(yi)*%DIMU% + uint(clamp(xi+d,0,W-1))"
                               : "uint(clamp(yi+d,0,W-1))*%DIMU% + uint(xi)";
  std::string storages =
  ...
  if (!horizontal) {
    ...
    storages +=
      "storage_interface si_g (descriptor_set 0) { buffer layout(std430) gb { float gdata[%DIMSQ%]; }; }\n";
...
struct LpfModuleInst : public TerrainComputeInst {
  ...
  void _fillParams(BakeEnv* env) {
    ...
    float P[4]   = {inv2s2, blend, 0.0f, 0.0f};
  ...
  void onActivate(dflow::GraphInst* inst) final {
    ...
    _csH = fxi->computeShader(fxi->shaderFromShaderText("terrain_lpf_h", _lpf_text(dim, "cs_lpf_h", true,  _rmax)), "cs_lpf_h");
    _csV = fxi->computeShader(fxi->shaderFromShaderText("terrain_lpf_v", _lpf_text(dim, "cs_lpf_v", false, _rmax)), "cs_lpf_v");
