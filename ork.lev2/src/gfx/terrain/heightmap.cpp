...
HeightMap::HeightMap(int isx, int isz)
  ...
  fvec3 HeightMap::ComputeNormal(int ix1, int iz1) const {
    ...
    fvec3 d0 = (XYZ(ix0, iz0) - VC).normalized();
    fvec3 d1 = (XYZ(ix1, iz0) - VC).normalized();
    fvec3 d2 = (XYZ(ix2, iz0) - VC).normalized();
    fvec3 d3 = (XYZ(ix2, iz1) - VC).normalized();
    fvec3 d4 = (XYZ(ix2, iz2) - VC).normalized();
    fvec3 d5 = (XYZ(ix1, iz2) - VC).normalized();
    fvec3 d6 = (XYZ(ix0, iz2) - VC).normalized();
    fvec3 d7 = (XYZ(ix0, iz1) - VC).normalized();
  ...
  bool HeightMap::Load(const ork::file::Path& pth) {
    ...
    printf("HeightMap<%p> hfmin<%d> hfmax<%d>\n", (void*)this, int(hfmin), int(hfmax));
    printf(
        "HeightMap<%p> timer<%g> abs_path<%s> exists<%d>\n", (void*)this, timer.SecsSinceStart(), abs_path.c_str(), int(bexists));
    return bexists;
  }
