...
LoadJoinSet::~LoadJoinSet() {
  // Fallback for join-less usage (destructed before join()); idempotent with
  // the join()-time release via the exchange.
  if (_marker_active.exchange(false))
    asyncWorkEnd(_name);
}

///////////////////////////////////////////////////////////////////////////////

void LoadJoinSet::_ensureMarker() {
  bool expected = false;
  if (_marker_active.compare_exchange_strong(expected, true))
    asyncWorkBegin(_name); // re-armed wave after a join() released the marker
}
...
void LoadJoinSet::join(Context* ctx) {
  ...
  // The wave is JOINED — release the async-tracker marker now. Deferring it to
  // the destructor pins asyncWorkPending()>0 for the lifetime of a member-held
  // set (the sim's _loadJoinSet), which starves the offscreen settle gate once
  // any other load (e.g. the v1b eager skybox) holds settle past this
  // rendezvous. Re-arm re-acquires via _ensureMarker().
  if (_marker_active.exchange(false))
    asyncWorkEnd(_name);
}
