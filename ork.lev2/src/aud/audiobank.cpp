...
class AudioBankLoader final : public ork::asset::FileAssetLoader {
public:
  AudioBankLoader();
  ...
  void DestroyAsset(asset::asset_ptr_t asset) override {
    auto bank = std::dynamic_pointer_cast<AudioBank>(asset);
    AudioDevice::GetDevice()->DestroyBank(bank.get());
  }
};
...
bool AudioBankLoader::LoadFileAsset(asset::asset_ptr_t pAsset, ConstString filename) {
  auto bank = std::dynamic_pointer_cast<AudioBank>(pAsset);
  bool bOK  = AudioDevice::GetDevice()->LoadSoundBank(bank.get(), filename);
  OrkAssert(bOK);
  return true;
}
...
AudioInstrumentPlayback* AudioDevice::PlaySound(AudioProgram* Program, const AudioIntrumentPlayParam& PlaybackParams) {
  ...
  if (PlaybackHandle != 0) {
    ...
    if (Program) {
      ...
      for (int ipzone = 0; ipzone < inumpzones; ipzone++) {
        ...
        //////////////////////////////////////
        if ((inote >= pzone.GetKeyMin()) && (inote <= pzone.GetKeyMax()))
        //////////////////////////////////////
        {
          ...
          for (int iiz = 0; iiz < inumizones; iiz++) {
            ...
            //////////////////////////////////////
            if ((inote >= izone.GetKeyMin()) && (inote <= izone.GetKeyMax()))
            //////////////////////////////////////
            {
