...
StreamingOscillatorBlock::StreamingOscillatorBlock(const DspBlockData* dbd)
  ...
  logchan_streaming->log("STREAMING_OSC: Initialized with target latency %.1fms (%zu samples)", 
         _streamingdata->_target_latency_ms, _dynamic_target_level);
  logchan_streaming->log("STREAMING_OSC: Watermarks - Low: %zu, Target: %zu, High: %zu",
         _dynamic_low_watermark, _dynamic_target_level, _dynamic_high_watermark);
  logchan_streaming->log("STREAMING_OSC: Ring buffer capacity: %zu samples (%.1f seconds)",
