...
#include <ork/lev2/aud/singularity/PolyBLEP.h>
...
template <typename T> inline T square_number(const T& x) {
  return x * x;
}
...
template <typename T> inline int64_t bitwiseOrZero(const T& t) {
...
double PolyBLEP::getAndInc() {
  ...
  inc();
  double next = (sample * 0.707 + prv * 0.293);
  prv         = sample;
...
double PolyBLEP::tri2() const {
  ...
  y += freqInSecondsPerSample / (pulseWidth - pulseWidth * pulseWidth) *
       (blamp(t1, freqInSecondsPerSample) - blamp(t2, freqInSecondsPerSample));
...
double PolyBLEP::trip() const {
  ...
  if (pulseWidth > 0) {
    ...
    t3 -= bitwiseOrZero(t3);
    y += 2 * freqInSecondsPerSample / pulseWidth *
         (blamp(t1, freqInSecondsPerSample) - 2 * blamp(t2, freqInSecondsPerSample) + blamp(t3, freqInSecondsPerSample));
  }
  return amplitude * y;
}
