...
void filescanner::ParseKeyMapEntryIt(Keymap* kmap, const datablock& db, datablock::iterator& it, int ivelrng) {
  ...
  // gxmlout += CreateFormattedString("	<range r='%s' inument='%d' et_2btune='%d' et_1btune='%d' et_voladj='%d' et_samp='%d'
...
void filescanner::ParseKeyMap(const datablock& db, datablock::iterator& it, int iObjectID, std::string ObjName) {
  ...
  // ObjName.c_str(), uSampleHeaderID, uNumberOfEntries, uEntrySize, uMethod ); printf( "  BasePitch<%d> CentsPerEntry<%d>\n",
  // uBasePitch, uCentsPerEntry );

  // gxmlout += CreateFormattedString("<keymap id='%d' name='%s' sampid='%d' meth='%x' basep='%x' >\n", iObjectID, ObjName.c_str(),
  // int(uSampleHeaderID), int(uMethod), int(uBasePitch) );
    ...
    for (int ivel = 0; ivel < 8; ivel++) {
      for (int ikey = 0; ikey < 128; ikey++) {
        ...
        if (bmatch) // found region
        {
          ...
          int iactualsampid = (refrd->miSampleId == 0) ? ikmsampid : refrd->miSampleId;
