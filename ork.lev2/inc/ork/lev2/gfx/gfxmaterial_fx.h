...
public:
  ...
  FxParamRec& GetRecord() {
    return mRecord;
  }
  const FxParamRec& GetRecord() const {
    return mRecord;
  }
  ...
  bool IsBindable(void) const {
    return mbIsBindable;
  }
  void SetBindable(bool bv) {
    mbIsBindable = bv;
  }

  const std::string& GetInitString() const {
    return mParameterInitString;
  }
  void SetInitString(const std::string& initstr) {
    mParameterInitString = initstr;
  }

  GfxMaterialFx* GetParentMaterial() const {
    return mParentMaterial;
  }
  orklut<std::string, std::string>& RefAnnotations() {
    return mAnnotations;
  }
  const orklut<std::string, std::string>& RefAnnotations() const {
    return mAnnotations;
  }
...
public:
  GfxMaterialFxParam(GfxMaterialFx* parent = NULL)
      : GfxMaterialFxParamBase(parent) {
  }
...
public:
  ...
  void AddParameter(GfxMaterialFxParamBase* param);
  std::string GetParamValue(const std::string& pname) const {
    return mEffectInstance.GetParamValue(pname);
  }

  const orkvector<VertexConfig>& RefVertexConfig(void) const {
    return mVertexConfigData;
  }

  const GfxMaterialFxEffectInstance& GetEffectInstance() const {
