...
using parser_ptr_t = std::shared_ptr<OrkSlParser>;
...
using parser_constptr_t = std::shared_ptr<const OrkSlParser>;
...
struct FnParseContext {
  FnParseContext(OrkSlParser* parser, const ScannerView* v);
...
struct ParsedFunctionNode : public AstNode {
  ...
  int parse(OrkSlParser* parser, const ScannerView& view);
...
struct NamedBlockNode : public AstNode {
  ...
  void parse(OrkSlParser* parser, const ScannerView& view);
...
struct PassNode : public NamedBlockNode {
  ...
  int parse(OrkSlParser* parser, const ScannerView& view, int start);
...
struct InterfaceNode : public DecoBlockNode {
  InterfaceNode(EStreamInterfaceType type)
      : DecoBlockNode()
      , _interfaceType(type) {
  ...
  void parse(OrkSlParser* parser, const ScannerView& view);
  void parseIos(OrkSlParser* parser, const ScannerView& view, iocontainernode_ptr_t ioc);
...
struct VertexInterfaceNode : public InterfaceNode {
  explicit VertexInterfaceNode()
      : InterfaceNode(EStreamInterfaceType::Vertex) {
  }
...
struct ComputeInterfaceNode : public InterfaceNode {
  explicit ComputeInterfaceNode()
      : InterfaceNode(EStreamInterfaceType::Compute) {
  }
  void _generate1(shaderbuilder::BackEnd& backend) const final;
};
#endif
...
struct NvTaskInterfaceNode : public InterfaceNode {
  explicit NvTaskInterfaceNode()
      : InterfaceNode(EStreamInterfaceType::NvTask) {
  }
  void _generate1(shaderbuilder::BackEnd& backend) const final;
};
struct NvMeshInterfaceNode : public InterfaceNode {
  explicit NvMeshInterfaceNode()
      : InterfaceNode(EStreamInterfaceType::NvMesh) {
  }
  void _generate1(shaderbuilder::BackEnd& backend) const final;
};
