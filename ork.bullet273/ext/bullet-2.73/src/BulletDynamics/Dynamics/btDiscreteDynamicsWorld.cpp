...
m_gravity(0,-10,0),
m_localTime(btScalar(1.)/btScalar(240.)),
m_profileTimings(0)
{
	...
	if (numSimulationSubSteps)
	{
		...
		for (int i=0;i<clampedSimulationSteps;i++)
		{
			if(getDebugDrawer() && getDebugDrawer()->getDebugMode())
				getDebugDrawer()->beginInternalStep();

			internalSingleStepSimulation(fixedTimeStep);

			if(getDebugDrawer() && getDebugDrawer()->getDebugMode())
				getDebugDrawer()->endInternalStep();
...
void	btDiscreteDynamicsWorld::internalSingleStepSimulation(btScalar timeStep)
{
	...
	static btScalar accum = 0.0f;
	counter++;
	totalcounter++;
	accum += timeStep;
	if(accum >= 1.0f)
	{
		printf("internalSingleStepSimulation counter = %d, totalcounter = %d\n", counter, totalcounter);
		accum -= 1.0f;
		counter = 0;
	}
#endif
	...
	// SCG MODS - clear forces each tick because we add forces in callback (see GenerateForces)
	clearForces();
...
static void debugDrawCircle(const btVector3 &center, const btVector3 &x, const btVector3 &y, const btVector3 &color, btIDebugDraw *debug) {
	btVector3 prev;
	const int segs = 12;
	float step = 2*3.141593f/segs;
	float at = 0;

	prev = center+cos(-step)*x+sin(-step)*y;
	for(int i=0 ; i<segs ; i++, at+=step) {
		btVector3 next = center+cos(at)*x+sin(at)*y;
...
void btDiscreteDynamicsWorld::debugDrawSphere(btScalar radius, const btTransform& transform, const btVector3& color)
{
	...
	// XY 
	debugDrawCircle(start, xoffs, yoffs, color, getDebugDrawer());

	// XZ
	debugDrawCircle(start, xoffs, zoffs, color, getDebugDrawer());

	// YZ
	debugDrawCircle(start, yoffs, zoffs, color, getDebugDrawer());
}
