...
class _Automoc:
    ...
    def __automoc_strategy_simple(self, env, moc_options, 
        ...
        if h and self.qo_search.search(h_contents):
            ...
            moc_name = "mocn1_" + self.splitext(cpp.name)[0] + ".cpp"
            moc_cpp = env.XMoc4(target=moc_name, source=h)
            moc_o = self.objBuilder(moc_cpp)
            out_sources.extend(moc_o)
            if moc_options['debug']:
                print "PTHN1>> scons: qt4: found Q_OBJECT macro in '%s', moc'ing to '%s'" % (str(h), str(moc_cpp))
        if cpp and self.qo_search.search(cpp_contents):
            # cpp file with Q_OBJECT macro found -> add moc
            # (to be included in cpp)
            cpp_path = str(cpp)
            cpp_fold,cpp_name = os.path.split(cpp_path)
            moc_name = cpp_fold+"/" + self.splitext(cpp_name)[0] + ".moc"
            moc_cpp = env.Moc4(target=moc_name, source=cpp)
            #moc_o = self.objBuilder(moc_cpp)
            #out_sources.extend(moc_o)
            #moc = env.Moc4(cpp)
            #env.Ignore(moc, moc)
            if moc_options['debug']:
                print "PTHN2>> scons: qt4: found Q_OBJECT macro in '%s', moc'ing to '%s'" % (str(cpp), str(moc_cpp))
    ...
    def __automoc_strategy_include_driven(self, env, moc_options,
        ...
        of the qtsolutions...
        """
        #print "autoscan inx cxx_suffixes<%s>" % cxx_suffixes
        if self.splitext(str(cpp))[1] in cxx_suffixes:
            ...
            ######################################
            include_re = re.compile(r'#include\s*[<,"][\w|/|.]*[>,"]', re.M)
            inc_h_moc = r'#include\s+"%s"' % h_moc
            inc_cxx_moc = r'#include\s+"%s"' % cxx_moc
            #print "cpp<%s>" % cpp
            #print "h_moc<%s>" % h_moc
            #print "cxx_moc<%s>" % cxx_moc
            includes = include_re.findall(cpp_contents);

            #print "includes<%s>" % includes
            ######################################
            # search for header of same name in incpaths
            ######################################
            if cpp:
              for inc in includes:
                print "inc2<%s> h_moc<%s> cxx_moc<%s>" % (inc,h_moc,cxx_moc)
                if re.search(pureh, inc):
                  #print "inc<%s>" % inc
                  inc = string.replace(inc,'\t',"")
                  inc = string.replace(inc,' ',"")
                  inc = string.replace(inc,'#include',"")
                  inc = string.replace(inc,'<',"")
                  inc = string.replace(inc,'>',"")
                  inc = string.replace(inc,'"',"")
                  h = find_file(inc, [cpp.get_dir()]+moc_options['cpppaths'], env.File)
                  if h:
                        if moc_options['debug']:
                            print "scons: qt4: Scanning '%s' (header of '%s')" % (str(h), str(cpp))
                        h_contents = h.get_contents()
                  ...
                  if h and self.qo_search.search(h_contents):
                    ...
                    #            break
                    if moc_options['debug']:
                        print "PTH0>> scons: qt4: found Q_OBJECT macro in '%s', moc'ing to '%s'" % (str(h), str(h_moc))
                  else:
                    if moc_options['debug']:
                    ...
                    for idx, s in enumerate(out_sources):
                        if hasattr(s, "sources") and len(s.sources) > 0:
                            if str(s.sources[0]) == h_moc:
                                ...
                                break
                    if moc_options['debug']:
                        print "PTH1>> scons: qt4: found Q_OBJECT macro in '%s', moc'ing to '%s'" % (str(h), str(h_moc))
                else:
                    if moc_options['debug']:
                        print "scons: qt4: found no Q_OBJECT macro in '%s', but a moc'ed version '%s' gets included in '%s'" % (str(h), inc_h_moc, cpp.name)
            ...
            if cpp and re.search(inc_cxx_moc, cpp_contents):
                ...
                if self.qo_search.search(cpp_contents):            
                    moc_name = "mocp2_"+item + ".cpp"
                    moc = env.XMoc4(target=moc_name, source=cpp)
                    #env.Ignore(moc, moc)
                    added = True
                    if moc_options['debug']:
                        print "PTH2>> scons: qt4: found Q_OBJECT macro in '%s', moc'ing to '%s'" % (str(cpp), str(moc))
                else:
                    if moc_options['debug']:
    ...
    def __call__(self, target, source, env):
        ...
        for obj in source:
            ...
            if not obj.has_builder():
                ...
                continue
            cpp = obj.sources[0]
            print "CCPP<%s> tgt<%s>" % (cpp,str(target[0]))
...
#    env.SetDefault(
    env.Replace(
        ...
        #QT4_XMOCHPREFIX = 'moc_h_',
        #QT4_XMOCHSUFFIX = '.moc',
        #QT4_XMOCCXXPREFIX = 'moc_cxx_',
...
def enable_modules(self, modules, debug=False, crosscompiling=False) :
    ...
    validModules = [
        ...
        'Qt5Gui',
        'Qt5OpenGL',
	# hlh: the following modules no longer appear to be in Qt5
        #'Qt3Support',
	...
	# hlh: FIXME there are a bunch of new modules in Qt5--figure out what to do?
	'Qt5Widgets',
        ]
    pclessModules = [
...
#        'QtDesignerComponents',
    ...
    moduleDefines = {
        ...
        'QtCore'     : ['QT_CORE_LIB'],
        'QtPlatformSupport'     : ['QT_PLATFORMSUPPORT_LIB'],
    }
    for module in modules :
