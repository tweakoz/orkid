###############################################################################
# mtl_pine.py — procedural Pine on a sphere (GEOV2 ptex3d, volumetric wood grain).
#
...
#
#   ork.scene.viewer.py mtl_pine
###############################################################################

from orkengine.core import lev2_pyexdir, vec3
from ork.hypergraph.ecs.scene import Scene
from ork.hypergraph.assets.materials import Pine

lev2_pyexdir.addToSysPath()


class PineScene(Scene):

  def __init__(self):
    super().__init__()                       # self.SG = default ForwardPBR scenegraph
    A = self.asset

    mat = A.Ptex3d("pine_mtl",
                   dsl_class=Pine)
    drw = A.IcoSphere("pine_sphere",
                      radius=2.5,
                      subdivisions=6,
                      material=mat)
