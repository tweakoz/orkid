...
float4 ps_comic( Fragment FragIn ) : COLOR
{
	...
	float2 Uv0 = float2( kftexX+FragIn.Uv0.x, kftexY+(1.0f-FragIn.Uv0.y) );
...
{
	const float kftexX = 1.0f/1024.0f;
	const float kftexY = 1.0f/1024.0f;
...
technique frameeffect_fbout
{
    ...
    {
        VertexShader = compile vs_3_0 vs_basic();
        PixelShader = compile ps_3_0 ps_fbout();
    }
}
...
technique frameeffect_fbin
{
    ...
    {
        VertexShader = compile vs_3_0 vs_basic();
        PixelShader = compile ps_3_0 ps_fbin();
    }
}
