...
libblock lib_fwd_impl
  ...
  pbr._albedo = albedo;
  pbr._metallic = mtlrufambtex.z;
  pbr._roughness = mtlrufambtex.y;
...
libblock lib_fwd     //
  ...
  vec3 _forward_lighting(vec3 modcolor, vec3 eyepos) {
    ...
    vec3 mtlrufambtex = texture(CNMREA, vec3(frg_uv0, 2)).xyz;
